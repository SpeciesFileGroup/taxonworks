...
describe GeographicAreasHelper, :type => :helper do
  context 'a geographic_area needs some helpers' do
    before(:all) {
      ...
      @cvt_name = @geographic_area.name
    }

    specify '::geographic_area_tag' do
      expect(GeographicAreasHelper.geographic_area_tag(@geographic_area)).to eq(@cvt_name)
    end

    specify '#geographic_area_tag' do
      expect(geographic_area_tag(@geographic_area)).to eq(@cvt_name)
    end

    specify '#geographic_area_link' do
      expect(geographic_area_link(@geographic_area)).to have_link(@cvt_name)
    end
