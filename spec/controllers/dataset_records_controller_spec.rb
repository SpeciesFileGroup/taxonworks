...
RSpec.describe DatasetRecordsController, type: :controller do
  ...
  xdescribe "POST #create" do # TODO: Might be useful to add new rows.
    context "with valid params" do
      it "creates a new DatasetRecord" do
        expect {
          post :create, params: {dataset_record: valid_attributes}, session: valid_session
        }.to change(DatasetRecord, :count).by(1)
      end

      it "redirects to the created dataset_record" do
        post :create, params: {dataset_record: valid_attributes}, session: valid_session
        expect(response).to redirect_to(DatasetRecord.last)
      end
    end

    context "with invalid params" do
      it "returns a success response (i.e. to display the 'new' template)" do
        post :create, params: {dataset_record: invalid_attributes}, session: valid_session
        expect(response).to be_successful
      end
    end
  end
  ...
  xdescribe "DELETE #destroy" do # TODO: Might be useful to delete rows.
    it "destroys the requested dataset_record" do
      dataset_record = DatasetRecord.create! valid_attributes
      expect {
        delete :destroy, params: {id: dataset_record.to_param}, session: valid_session
      }.to change(DatasetRecord, :count).by(-1)
    end

    it "redirects to the dataset_records list" do
      dataset_record = DatasetRecord.create! valid_attributes
      delete :destroy, params: {id: dataset_record.to_param}, session: valid_session
      expect(response).to redirect_to(dataset_records_url)
    end
  end
