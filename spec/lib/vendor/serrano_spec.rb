...
describe TaxonWorks::Vendor::Serrano, type: :model, group: [:sources] do
  ...
  context '#cached_string correctly formats ' do
    ...
    specify 'text1' do
      expect(src1.cached_string('text')).to eq('Brauer, A. (1909) Die Süsswasserfauna Deutschlands. Eine Exkursionsfauna bearb. ... und hrsg. von Dr. Brauer. G. Fischer, https://doi.org/10.5962%2Fbhl.title.1086')
    end
    ...
    specify 'text2' do
      expect(src2.cached_string('text')).to eq(
        'Kevan, D.K.M.E. & Wighton, D.C. (1981) Paleocene orthopteroids from south-central Alberta, Canada. Canadian Journal of Earth Sciences, 18(12), 1824–1837. https://doi.org/10.1139%2Fe81-170'
      )
    end

    specify 'html1' do
      expect(src1.cached_string('html')).to eq(
        'Brauer, A. (1909) <i>Die Süsswasserfauna Deutschlands. Eine Exkursionsfauna bearb. ... und hrsg. von Dr. Brauer.</i> G. Fischer, https://doi.org/10.5962%2Fbhl.title.1086')
    end
    ...
    specify 'html2' do
      expect(src2.cached_string('html')).to eq(
        'Kevan, D.K.M.E. &amp; Wighton, D.C. (1981) Paleocene orthopteroids from south-central Alberta, Canada. <i>Canadian Journal of Earth Sciences</i>, 18(12), 1824–1837. https://doi.org/10.1139%2Fe81-170'
      )
    end
  end
