...
describe Queries::Confidence::Filter, type: :model do
  ...
  specify '#polymorphic_id 1' do
    ...
    expect(query.all.to_sql).to eq("SELECT \"confidences\".* FROM \"confidences\" WHERE \"confidences\".\"confidence_object_id\" = 1 AND \"confidences\".\"confidence_object_type\" = 'CollectingEvent' AND (\"confidences\".\"project_id\" = 1)")
  end
