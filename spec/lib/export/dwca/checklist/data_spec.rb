  ...
  context 'when initialized with a scope' do
    ...
    context 'with some occurrence records created' do
      ...
      context 'with distribution extension' do
        # Override with fresh OTUs using cached taxon names
        let(:taxon_name1) { TaxonName.find(cached_ids[:taxon_name1]) }
        let(:taxon_name2) { TaxonName.find(cached_ids[:taxon_name2]) }
        let(:taxon_name3) { TaxonName.find(cached_ids[:taxon_name3]) }
        let!(:otu1) { FactoryBot.create(:valid_otu, taxon_name: taxon_name1) }
        let!(:otu2) { FactoryBot.create(:valid_otu, taxon_name: taxon_name2) }
        let!(:otu3) { FactoryBot.create(:valid_otu, taxon_name: taxon_name3) }
        let(:otu_scope) { { otu_id: [otu1.id, otu2.id, otu3.id] } }
      ...
      context 'with vernacular name extension' do
        ...
        # Create specimens for OTUs so they generate DwC occurrences
        before do
