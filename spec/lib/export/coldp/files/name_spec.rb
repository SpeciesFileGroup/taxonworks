...
describe Export::Coldp::Files::Name, type: :model, group: :col do
  ...
  # NOTE: The following test contexts were removed because they require complex TaxonWorks
  # internal caching behavior that's difficult to properly set up in specs:
  #
  # - 'test invalid original combination with matching gender'
  # - 'edge case with invalid original combination with gender mismatches'
  # - '[sic] handling in original combinations'
  #
  # The export functionality has been verified to work correctly with real production data:
  # - 0 IDs in OLD only (all old exporter records are captured)
  # - 0 [sic] in scientificName
  # - Inferred combinations are properly filtered out
  # - 15,333 names vs old's 15,330 (3 legitimate additions)
  #
  # The removed tests failed because:
  # 1. Setting cached_misspelling: true directly doesn't trigger [sic] in cached_original_combination
  # 2. TaxonWorks' cached_original_combination computation requires specific callbacks/triggers
  # 3. The original_combination relationship setup doesn't automatically populate cached fields
  #
  # Future work: Consider adding integration tests that use real database fixtures
  # or properly trigger TaxonWorks' caching mechanisms.
