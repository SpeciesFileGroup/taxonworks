module Features
  module FormHelpers
    ...
    def fill_autocomplete(field, options = {})
      ...
      css_selector = %Q{li.autocomplete-item[data-model-id="#{options[:select]}"]}
    ...
    def fill_role_picker_autocomplete(field, options = {})
      ...
      css_selector = %Q{li.autocomplete-item a span[data-person-id="#{options[:select]}"]}
      fill_in field, with: options[:with]
      find(css_selector).hover.click
    end
    ...
    def fill_otu_widget_autocomplete(field, options = {})
      ...
      css_selector = %Q{li.autocomplete-item[id=ui-otu-id-#{options[:select]}]}
      fill_in field, with: options[:with]
      find(css_selector).hover.click
    end

    def fill_keyword_autocomplete(field, options = {})
      css_selector = %Q{li.autocomplete-item a span[data-tag-id="#{options[:select]}"]}
      fill_in field, with: options[:with]
      find(css_selector).hover.click
    end
    ...
    def fill_autocomplete_and_select(field, with: nil, select_id: nil, object_type: nil)
      ...
      css_selector = %Q{li.autocomplete-item a[data-#{object_type}-id="#{select_id}"]}
      fill_in field, with: with
      find(css_selector).hover.click
    end

    def fill_area_picker_autocomplete(field, options = {})
      css_selector = %Q{li.autocomplete-item a[data-geographic-area-id="#{options[:select]}"]}
      fill_in field, with: options[:with]
      find(css_selector).hover.click
    end
