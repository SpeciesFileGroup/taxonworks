...
describe ConfidenceLevel, type: :model, group: :confidences do
  ...
  context 'scopes' do
    ...
    specify '#used_recently' do
      expect(ConfidenceLevel.used_recently(k1.created_by_id, k1.project_id)).to contain_exactly(k1)
    end

    specify '#used_on_klass.used_recently' do
      expect(ConfidenceLevel.used_on_klass('Otu').used_recently(k1.created_by_id, k1.project_id)).to contain_exactly(k1)
    end
  end
