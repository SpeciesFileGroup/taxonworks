...
# Specs to verify that the Ruby and SQL implementations of media identifier
...
RSpec.describe Shared::Dwc::MediaIdentifier, type: :model do
  ...
  shared_examples 'media identifier consistency' do |media_class, table_alias, factory_name|
    context "for #{media_class.name}" do
      ...
      specify 'produces same result from Ruby and SQL when using UUID' do
        media = FactoryBot.create(factory_name)
        uuid_id = FactoryBot.create(:identifier_global_uuid, identifier_object: media, is_generated: true)
        uuid_value = uuid_id.identifier
      ...
      specify 'produces same result from Ruby and SQL when using URI (no UUID)' do
        media = FactoryBot.create(factory_name)
      ...
      specify 'produces same result from Ruby and SQL when using ID (no UUID or URI)' do
        media = FactoryBot.create(factory_name)
      ...
      specify 'produces same result when both UUID and URI are present (UUID takes precedence)' do
        media = FactoryBot.create(factory_name)
