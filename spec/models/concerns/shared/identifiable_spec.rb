...
describe 'Identifiable', :type => :model do
  ...
  context "methods" do
    ...
    let!(:identifier1) { FactoryGirl.create(:valid_identifier, identified_object: o, identifier: '123', namespace: n1) } 
    let!(:identifier2) { FactoryGirl.create(:valid_identifier, identified_object: o, identifier: '456', namespace: n2) } 
    let!(:identifier3) { FactoryGirl.create(:valid_identifier, identified_object: o, identifier: '789', namespace: n3) } 
    ...
    specify "#with_namespaced_identifier(namespace_name, identifier)" do
      ...
      expect(identifiable_class.with_namespaced_identifier(namespace_name, '123').count).to eq(1)
    end
