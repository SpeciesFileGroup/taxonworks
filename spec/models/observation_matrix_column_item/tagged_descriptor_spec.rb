...
RSpec.describe ObservationMatrixColumnItem::TaggedDescriptor, type: :model, group: :observation_matrix  do
  ...
  context 'validation' do
    ...
    specify 'type is "MatrixColumnItem::TaggedDescriptor"' do
      expect(observation_matrix_column_item.type).to eq('ObservationMatrixColumnItem::TaggedDescriptor')
    end
    ...
    context 'with a observation_matrix_column_item saved' do
      ...
      before {
        observation_matrix_column_item.controlled_vocabulary_term = keyword
      ...
      context 'overlapping sets' do
        ...
        context 'adding another tag to an existing cvt' do
          ...
          specify 'observation_matrix column is added' do
            expect(ObservationMatrixColumn.all.map(&:descriptor).map(&:metamorphosize)).to contain_exactly(descriptor1, descriptor2, descriptor3, descriptor4)
          end
      ...
      specify 'keyword/controlled vocabulary term can only be added once to observation_matrix_column_item' do
        expect(ObservationMatrixColumnItem::TaggedDescriptor.new(observation_matrix: observation_matrix, controlled_vocabulary_term: keyword).valid?).to be_falsey
      end
