...
RSpec.describe Observation::Qualitative, type: :model, group: :observation_matrix do
  ...
  context 'validation' do
    ...
    context 'duplication' do
      ...
      let!(:o1) { FactoryBot.create(:valid_observation, character_state_id: cs.id, descriptor: descriptor, otu: otu, type: 'Observation::Qualitative') }

      specify 'are prevented 1' do
        o = Observation.create(otu: otu, descriptor: descriptor, character_state: cs, type: 'Observation::Qualitative')
        expect(o.errors.include?(:character_state_id)).to be_truthy
      end

      specify 'are prevented 2' do
        o = Observation.create(otu: otu, descriptor: descriptor, character_state_id: cs.id, type: 'Observation::Qualitative')
        expect(o.errors.include?(:character_state_id)).to be_truthy
      end
    end
  end
