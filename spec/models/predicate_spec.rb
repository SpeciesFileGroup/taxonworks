...
describe Predicate, type: :model do
  ...
  context 'scopes' do
    ...
    specify '.used_recently' do
      expect(Predicate.used_recently(predicate.created_by_id, predicate.project_id)).to contain_exactly(predicate)
    end

    specify '#used_recently.used_on_klass 1' do
      expect(Predicate.used_recently(predicate.created_by_id, predicate.project_id).used_on_klass('Otu')).to contain_exactly(predicate)
    end

    specify '#used_recently.used_on_klass 1' do
      expect(Predicate.used_recently(predicate.created_by_id, predicate.project_id).used_on_klass('CollectionObject')).to contain_exactly()
    end
