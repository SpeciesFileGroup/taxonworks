...
describe Protonym, type: :model, group: [:nomenclature, :protonym] do
  ...
  # Creating a TNR does not reload the related object anymore
  specify 'misspelled original combination 1' do
    ...
    expect(alternate_genus.reload.cached_original_combination).to eq('Bus [sic]')
  end

  specify 'misspelled original combination 2' do
    alternate_genus.update!(iczn_set_as_misspelling_of: genus, original_genus: alternate_genus)
    expect(alternate_genus.reload.cached_original_combination_html).to eq('<i>Bus</i> [sic]')
  end

  specify 'misspelled original combination 3' do
    alternate_genus.update!(iczn_set_as_misspelling_of: genus, original_genus: alternate_genus)
    species.update!(original_genus: alternate_genus) # touching this updates cached, so this differs from above 2 specs
