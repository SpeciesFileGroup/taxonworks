...
=end
  ...
  context :validation do
    let(:valid_matrix_given) do
      observation_matrix_row.observation_matrix = FactoryGirl.create(:valid_observation_matrix)
    end

    let(:valid_otu_given) do
      observation_matrix_row.otu = FactoryGirl.create(:valid_otu)
    end

    let(:valid_collection_object_given) do
      observation_matrix_row.collection_object = FactoryGirl.create(:valid_collection_object)
    end
    ...
    context :invalid_observation_matrix_row do
      ...
      context 'with valid matrix given' do
        ...
        specify 'creating 2 MatrixRow with same matrix and same otu' do
          matrix = FactoryGirl.create(:valid_observation_matrix)
