      ...
      types: {
        ...
        DataAttribute: {
          attributes: [
            ...
            ['controlled_vocabulary_term', 'definition'],
            '',
            'created_by',
            'created_at'
          ]
  ...
  methods: {
    setAuthorsToList(list) {
      list.forEach((item, index) => {
        const member = this.membersList.find(
          (o) => o.user.id === item.created_by_id
        )
    ...
    removeItem(item) {
      AjaxCall('delete', `${item.object_url}.json`).then((response) => {
        const index = this.listWithCreators.findIndex(
          (obj) => obj.id === item.id && obj.base_class === item.base_class
        )
        ...
        TW.workbench.alert.create(
          'Annotation was successfully deleted.',
          'notice'
        )
      })
    },
