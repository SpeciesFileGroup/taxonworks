...
export default ({ state, commit, dispatch }, id) => {
  return new Promise((resolve, reject) => {
    GetDataset(id).then(
      ({ body }) => {
        const { import_filters, import_retry_errored } = body?.metadata || {}
        ...
        if (importRunning) {
          commit(
            MutationNames.SetParamsFilter,
            Object.assign({}, state.paramsFilter, { filter: import_filters })
          )
          commit(
            MutationNames.SetSettings,
            Object.assign({}, state.settings, {
              retryErrored: !!import_retry_errored,
              importModalView: true
            })
          )
