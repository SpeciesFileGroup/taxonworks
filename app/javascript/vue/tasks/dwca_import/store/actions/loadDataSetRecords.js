...
export default ({ state, commit, getters }, page) => {
  return new Promise((resolve, reject) => {
    ...
    const currentPageOnVirtualPage = virtualPagination[state.currentPage]
      ? virtualPagination[state.currentPage].from + pagePosition
      : pagePosition
    const lastVirtualPage = virtualPagination[state.currentPage]
      ? virtualPagination[state.currentPage].to
      : undefined
    const loadPage = virtualPagination[state.currentPage]
      ? currentPageOnVirtualPage
      : pagePosition === 0
      ? 1
      : pagePosition
    ...
    if (
      ...
      (loadPage && loadPage > lastVirtualPage) ||
      state.temporary.downloadingPages.includes(pagePosition)
    )
      return resolve()
    ...
    GetDatasetRecords(state.dataset.id, {
      ...
      (response) => {
        ...
        if (page === undefined) {
          page = 1
          commit(
            MutationNames.SetDatasetRecords,
            createEmptyPages(
              getters[GetterNames.GetVirtualPages][state.currentPage]
            )
          )
        }
        page = page - 1
        ...
        const index = state.temporary.downloadingPages.findIndex(
          (num) => num === pagePosition
        )
        if (index > -1) {
