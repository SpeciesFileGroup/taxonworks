<template>
  <div class="flex-wrap-column middle">
    ...
    <div class="horizontal-center-content full_width middle margin-small-top">
      <property-box v-model="subject" />
      <relationship-box
        :flip="flip"
        v-model="biological_relationship"
      />
      <property-box v-model="object" />
    </div>
    <div
...
export default {
  ...
  computed: {
    ...
    objectString() {
      return this.object
        ...
        .map((item) => item?.biological_property?.name || item.name)
        .join(', ')
    },
  ...
  methods: {
    setBiologicalRelationship(newVal) {
      ...
      this.object = newVal.object_biological_relationship_types.map((item) => ({
        ...item,
        created: true
      }))
      this.subject = newVal.subject_biological_relationship_types.map(
    ...
    saveBiologicalRelationship() {
      const object = this.object
        ...
        .map((item) => ({
          ...
          biological_property_id: item?.biological_property_id || item.id
        }))
      ...
      const removed = this.getDestroyed(this.subject).concat(
        this.getDestroyed(this.object)
      )
      const data = this.biological_relationship

      data.biological_relationship_types_attributes = subject.concat(
        object,
        removed
      )

      const saveRecord = data.id
        ? BiologicalRelationship.update(data.id, {
            biological_relationship: data,
            extend
          })
      ...
      saveRecord.then((response) => {
        ...
        TW.workbench.alert.create(
          'Biological relationship was successfully saved.',
          'notice'
        )
      })
    },
    ...
    getDestroyed(value) {
      return value
        .filter((item) => item?._destroy)
        .map((item) => ({ _destroy: item._destroy, id: item.id }))
    },
