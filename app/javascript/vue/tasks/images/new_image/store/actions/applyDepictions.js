...
export default ({ state, commit }) => {
  ...
  function createNewCOForStage() {
    return state.objectsForDepictions.length === 0 && state.sqed.layout
  }

  state.settings.saving = true
  if (createNewCOForStage()) {
    state.imagesCreated.forEach((item) => {
      collection_object.data_attributes_attributes = state.data_attributes.map(
        (item) => ({
      ...
      collection_object.tags_attributes = tags.map((tag) => ({
        keyword_id: tag.id
      }))
      ...
      promises.push(
        CollectionObject.create({ collection_object }).then((response) => {
          const depiction = {
            ...
            image_id: item.id,
            caption: state.depiction.caption.length
              ? state.depiction.caption
              : undefined,
            sqed_depiction_attributes: validateSqed(state.sqed)
              ? state.sqed
              : undefined
          }
      ...
      state.imagesCreated.forEach((item) => {
        const depiction = {
          ...
          sqed_depiction_attributes:
            validateSqed(state.sqed) &&
            state.objectsForDepictions.length === 1 &&
            object.base_class === 'CollectionObject'
              ? state.sqed
              : undefined
        }

        alreadyCreated = state.depictionsCreated.find(
          (depictionCreated) =>
        ...
        if (!alreadyCreated) {
          ...
          if (
            depiction.sqed_depiction_attributes &&
            alreadyCreated?.sqed_depiction?.id
          ) {
            depiction.sqed_depiction_attributes.id =
              alreadyCreated.sqed_depiction.id
          }
          promises.push(
