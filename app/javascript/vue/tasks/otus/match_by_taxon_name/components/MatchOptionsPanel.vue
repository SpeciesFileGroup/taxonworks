<template>
    <div class="match-options-panel panel content">
        <div class="flex-row flex-separate middle margin-medium-bottom">
            <h3>Match options</h3>
            <VBtn color="destroy" small @click="emit('clear-all')">
                Clear all matches
            </VBtn>
        </div>
        ...
        <div class="field margin-medium-bottom">
            ...
            >
                ...
                <VBtn circle color="primary" @click="clearScope">
        ...
        <div class="field margin-medium-bottom">
            ...
            <p class="subtle">
                When checked, OTUs for valid TaxonNames of synonyms are
                selected.
            </p>
        </div>
        ...
        <div class="field">
            ...
            >
                <input
                    ...
                    @change="
                        updateModifier(index, 'active', $event.target.checked)
                    "
                />
                <input
                    type="text"
                    ...
                    @input="
                        updateModifier(index, 'pattern', $event.target.value)
                    "
                />
                <input
                    type="text"
                    ...
                    "
                />
                <VBtn circle color="destroy" @click="removeModifier(index)">
                    <VIcon x-small name="trash" />
                </VBtn>
            </div>

            <div class="flex-row gap-small margin-small-top">
                <VBtn color="primary" small @click="addModifier">
                    Add row
                </VBtn>
                <select
                    class="normal-input"
                    @change="
                        addCommonReplacement($event.target.value);
                        $event.target.value = '';
                    "
                >
...
function addCommonReplacement(key) {
    ...
    modifiers.value = [
        ...modifiers.value,
        { active: true, pattern, replacement },
    ];
    debouncedUpdate();
}
</script>
