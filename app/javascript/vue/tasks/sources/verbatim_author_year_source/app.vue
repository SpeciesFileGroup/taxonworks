<template>
    <div id="verbatim-author-year-source-task">
        ...
        <div>
            <a :href="filterTaxonNamesUrl">Back to Filter TaxonNames</a>
        </div>

        <VSpinner v-if="store.isLoading" legend="Loading..." />
        ...
        <div v-else>
            <div class="flex-separate middle margin-medium-bottom">
                ...
                <div class="horizontal-left-content gap-small">
                    <VBtn color="primary" medium @click="jumpToMost">
                        Jump to most
                    </VBtn>
                    <VBtn color="primary" medium @click="refresh">
                        Refresh
                    </VBtn>
                </div>
            </div>
...
const filterTaxonNamesUrl = computed(() => {
    return RouteNames.FilterNomenclature;
});

onMounted(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const params = Object.fromEntries(urlParams.entries());
    store.loadAuthorYearData(params);
});
