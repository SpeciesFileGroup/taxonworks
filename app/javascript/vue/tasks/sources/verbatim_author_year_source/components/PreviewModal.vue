<template>
    <Modal @close="$emit('close')" container-style="max-width: 600px;">
        <template #header>
            <h3>{{ author }} {{ year }}</h3>
        </template>
        <template #body>
            <VSpinner v-if="isLoading" legend="Loading TaxonNames..." />
            ...
            <table v-else class="full_width">
                ...
                <tbody>
                    <tr v-for="taxonName in taxonNames" :key="taxonName.id">
                        <td>
                            {{
                                taxonName.cached ||
                                taxonName.name ||
