<template>
  <div>
    <fieldset v-help.section.BibTeX.authors>
      ...
      >
        <template #header>
          <div class="horizontal-left-content gap-small align-start">
            <RolePicker
              ref="rolePicker"
...
import IconUserRoundArrowLeft from '@/components/Icon/IconUserRoundArrowLeft.vue'
...
function reorderAuthorRoles(orderedIds) {
  ...
  for (const id of orderedIds) {
    const idx = remaining.findIndex((r) => (r.person_id ?? r.person?.id) === id)
