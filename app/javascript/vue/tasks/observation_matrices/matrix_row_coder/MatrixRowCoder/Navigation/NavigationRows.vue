...
const previousRowId = computed(
  () => store.getters[GetterNames.GetMatrixRow]?.previous_row?.id
)
const nextRowId = computed(
  () => store.getters[GetterNames.GetMatrixRow]?.next_row?.id
)
const observationMatrixId = computed(
  () => store.getters[GetterNames.GetMatrixRow]?.observation_matrix?.id
)

const rowLabels = ref([])

watch(observationMatrixId, () => {
