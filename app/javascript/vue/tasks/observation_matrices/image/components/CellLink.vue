...
import { OTU, COLLECTION_OBJECT, TAXON_NAME } from '@/constants/index.js'

const BROWSE_LINK = {
  [COLLECTION_OBJECT]: (id, _) =>
    `${RouteNames.BrowseCollectionObject}?collection_object_id=${id}`,
  [TAXON_NAME]: (id, _) =>
    `${RouteNames.BrowseNomenclature}?taxon_name_id=${id}`,
  [OTU]: (id, rowObject) =>
    rowObject.observation_matrix_id
...
export default {
  ...
  methods: {
    browseLink(object) {
      ...
      return klass in BROWSE_LINK ? BROWSE_LINK[klass](objectId, object) : ''
    }
  }
}
</script>
