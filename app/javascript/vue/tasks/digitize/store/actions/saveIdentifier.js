...
export default ({ commit, state }, id) =>
  new Promise((resolve, reject) => {
    ...
    if (
      ...
      state.settings.saveIdentifier
    ) {
      commit(
        MutationNames.SetIdentifierObjectId,
        state.container ? state.container.id : id
      )
      if (identifier.id) {
        Identifier.update(identifier.id, { identifier }).then(
          (response) => {
        ...
        if (!state.identifiers.length) {
          Identifier.create({ identifier }).then(
            (response) => {
              ...
              if (state.collection_object.id === id) {
                state.collection_object.object_tag =
                  state.identifier.identifier_object.object_tag
              } else {
                state.collection_objects[index].object_tag =
                  state.identifier.identifier_object.object_tag
              }
