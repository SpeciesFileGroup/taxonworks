...
const buildTaxonNamePayload = (taxon) => {
  const taxon_name = {
    id: taxon.id,
    name: taxon.name,
  ...
  if (Object.hasOwn(taxon, 'origin_citation_attributes')) {
...
export default ({ commit, state, dispatch }, taxon) => {
  ...
  const taxon_name = buildTaxonNamePayload(taxon)

  if (Object.hasOwn(taxon, 'origin_citation_attributes')) {
    delete state.taxon_name.origin_citation_attributes
  }

  return TaxonName.update(taxon_name.id, { taxon_name, extend })
    .then(
      (response) => {
        TW.workbench.alert.create(
          `Taxon name ${response.body.object_tag} was successfully updated.`,
          'notice'
        )

        if (!Object.hasOwn(response.body, 'taxon_name_author_roles')) {
