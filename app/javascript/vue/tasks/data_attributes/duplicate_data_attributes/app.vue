<template>
    <div class="task-container">
        <h1>Duplicate data attributes</h1>

        <VSpinner v-if="isLoading" full-screen legend="Loading duplicates..." />

        <div v-if="!hasQueryParam && !isLoading" class="panel content middle">
            <p>
                This task is accessed via the radial linker from a Filter
                result. No filter query was provided.
            </p>
        </div>
        ...
        <template v-if="hasData">
            ...
            <table class="table-striped full_width">
                <thead>
                    <tr>
                        <th class="w-4" @click="sortBy('object_tag')">
                            Object
                        ...
                        <th class="w-2" @click="sortBy('creator_name')">
                            Creator
                            <span v-if="sortColumn === 'creator_name'">{{
                                sortAscending ? "&uarr;" : "&darr;"
                            }}</span>
                        </th>
                        <th class="w-2" @click="sortBy('updater_name')">
                            Updater
                            <span v-if="sortColumn === 'updater_name'">{{
                                sortAscending ? "&uarr;" : "&darr;"
                            }}</span>
                        </th>
                        <th class="w-2" @click="sortBy('updated_at')">
                            Updated
                ...
                <tbody>
                    <template v-for="object in sortedObjects" :key="object.id">
                        <tr class="object-header-row">
                            <td colspan="6" v-html="object.object_tag" />
                            <td class="radial-cell">
                                <div class="horizontal-right-content gap-small">
                                    <RadialNavigator
                                        :global-id="object.global_id"
                                    />
                                    <RadialAnnotator
                                        :global-id="object.global_id"
                                    />
                        ...
                        >
                            ...
                            <td>
                                <span
                                    class="predicate-tag"
                                    :style="predicateStyle(da)"
                                    >{{ da.predicate_name }}</span
                                >
                            </td>
                            <td>
                                <textarea
                            ...
                            <td class="radial-cell">
                                <VBtn
                                    ...
                                    @click="
                                        () => destroyDataAttribute(object, da)
                                    "
                                >
...
function predicateStyle(da) {
    if (da.predicate_css_color) {
        return {
            backgroundColor: da.predicate_css_color,
            color: getContrastColor(da.predicate_css_color),
            padding: "2px 6px",
            borderRadius: "3px",
        };
    }
...
async function destroyDataAttribute(object, da) {
    ...
    try {
        ...
        if (object.data_attributes.length === 0) {
