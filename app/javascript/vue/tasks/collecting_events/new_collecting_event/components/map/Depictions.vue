      ...
      set(value) {
        if (value) {
          this.coordinatesEXIF.forEach((item) => {
            const found = this.queueGeoreferences.find(
          ...
          this.coordinatesEXIF.forEach((item) => {
            const index = this.queueGeoreferences.findIndex(
              (geo) => geo.tmpId === item.tmpId
            )
            if (index > -1) {
      ...
      handler(newVal, oldVal) {
        if (this.collectingEvent.id) {
          ...
          removed.forEach((item) => {
            const index = this.coordinatesEXIF.findIndex(
  ...
  methods: {
    ...
    addedfile(file) {
      EXIF.getData(file, () => {
        ...
        if (allMetaData?.GPSLatitude) {
          const coordinates = {
            latitude: ParseDMS(
              this.parseEXIFCoordinate(allMetaData.GPSLatitude) +
                allMetaData.GPSLatitudeRef
            ),
            longitude: ParseDMS(
              this.parseEXIFCoordinate(allMetaData.GPSLongitude) +
      ...
      if (window.confirm('Are you sure want to proceed?')) {
        Depiction.destroy(depiction.id).then((response) => {
          ...
          this.figuresList.splice(
            this.figuresList.findIndex((figure) => {
              return figure.id == depiction.id
            }),
            1
          )
    ...
    parseGeoJson(georeference) {
      const coordinates = JSON.parse(
        georeference.geographic_item_attributes.shape
    ...
    setExitDate(date) {
      if (
        !(
          this.collectingEvent.start_date_day ||
          this.collectingEvent.start_date_month ||
          this.collectingEvent.start_date_year
        )
      ) {
    ...
    setExifTime(time) {
      if (
        !(
          this.collectingEvent.time_start_hour ||
          this.collectingEvent.time_start_minute ||
          this.collectingEvent.time_start_second
        )
      ) {
