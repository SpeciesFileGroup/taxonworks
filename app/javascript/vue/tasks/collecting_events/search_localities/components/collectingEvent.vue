...
export default {
  ...
  methods: {
    compileList (newColEvList) {
      this.collectingEventList = this.append
        ? newColEvList.concat(this.collectingEventList)
    ...
    delistCE (index) {
      const ce_id = this.collectingEventList[index].id
      for (let i = this.featuresList.length - 1; i > -1; i--) {
    ...
    getGeoreferences () {
      let ce_ids = this.collectingEventList.map(ce => ce.id) // find the georeferences for these collecting_events

      this.featuresList = !this.append
        ? [].concat(this.shapes)
        : this.featuresList.concat(this.shapes)

      if (ce_ids.length) { // if the list has contents
        const cycles = (ce_ids.length / 30)  // each item is about 30 characters, make each cycle less than 2000 chars
        ...
        for (let i = 0; i < cycles; i++) {
          ...
          promises.push(this.makePromise({ collecting_event_ids: ce_ids.slice(thisCycle, endCycle) }))
          thisCycle += 30
        }
    ...
    makePromise (params) {
      return new Promise((resolve, reject) => {
        Georeference.where(params).then(response => {
          // put these geometries on the map as features
          const newFeatures = response.body.map(georeference => {
