...
export default {
  ...
  mounted() {
    this.getList(
      `/controlled_vocabulary_terms.json?type[]=BiocurationGroup`
    ).then((response) => {
      this.biocurationsGroups = response.body
      this.getList(
        `/controlled_vocabulary_terms.json?type[]=BiocurationClass`
      ).then((response) => {
  ...
  methods: {
    createBiocuration(id) {
      this.create(
        '/biocuration_classifications.json',
        this.createBiocurationObject(id)
      ).then((response) => {
        this.list.push(response.body)
      })
    },
    checkExist(id) {
      return this.list.some((bio) => id === bio.biocuration_class_id)
    },
    removeEntry(item) {
      this.removeItem(
        this.list.find((bio) => item.id === bio.biocuration_class_id)
      )
    },
    createBiocurationObject(id) {
      return {
        biocuration_classification: {
          ...
          biocuration_classification_object_id: this.metadata.object_id,
          biocuration_classification_object_type: this.metadata.object_type
        }
      }
    },
    splitGroups() {
