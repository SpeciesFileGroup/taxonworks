...
-->
<template>
    <div v-if="hasColLevel" class="col-dataset-picker">
        <label class="col-dataset-picker__label">ChecklistBank dataset</label>

        <!-- Current selection -->
        <div v-if="selectedDataset" class="col-dataset-picker__selected">
            <span class="col-dataset-picker__selected-title">{{
        ...
        <p v-else class="col-dataset-picker__default-note">
        ...
        <!-- Results -->
        <ul v-if="results.length > 0" class="col-dataset-picker__results">
            <li
            ...
            >
                <span class="col-dataset-picker__result-title">{{
                    dataset.title
                }}</span>
                <span
                    v-if="dataset.alias"
...
const selectedDataset = ref(
    currentSavedOptions.value?.dataset_id
        ? {
              ...
              title:
                  currentSavedOptions.value.dataset_title ||
                  currentSavedOptions.value.dataset_id,
              alias: null,
          }
        : null,
...
async function searchDatasets(q) {
    ...
    try {
        const { body } = await AjaxCall(
            "get",
            "/taxon_names/autoselect_col_datasets",
            { params: { q } },
        );
