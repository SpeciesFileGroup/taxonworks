    ...
    <div class="basic-information panel">
      ...
      <div class="body" v-show="expanded">
        <div class="separate-bottom">
          <div class="switch-radio">
            ...
            <label for="author-picker-source" class="">
              <span>Source</span>
              <div v-if="citation"><span class="small-icon icon-without-space" data-icon="ok"/></div>
            </label>
            ...
            <label for="author-picker-verbatim">
              Verbatim
              <div v-if="verbatimFieldsWithData"><span class="small-icon icon-without-space" data-icon="ok"/></div>
            </label>
            ...
            <label for="author-picker-person">
              <span>Person</span>
              <div v-if="roles.length">({{ roles.length }})<span class="small-icon icon-without-space" data-icon="ok"/></div>
            </label>
          </div>
        </div>
        <div v-if="show == 'source' && taxon.id">
          <div class="horizontal-left-content">
            <autocomplete
              ...
              display="label"/>
            <default-element v-if="!citation" label="source" type="Source" section="Sources" @getId="setSource"/>
          </div>
          <hr>
          <div v-if="citation != undefined">
            <div class="flex-separate middle">
              ...
              <citation-pages @setPages="addPages($event.origin_citation_attributes)" :citation="taxon"/>
              <span class="circle-button btn-delete" @click="removeSource(taxon.origin_citation.id)"/>
            </div>
          </div>
        </div>
        ...
        <div v-if="show == 'person'">
          <role-picker @create="updateTaxonName" @delete="updateTaxonName" v-model="roles" @sortable="updateTaxonName" @update="updatePersons" role-type="TaxonNameAuthor"/>
        </div>
      </div>
    </div>
  </form>
