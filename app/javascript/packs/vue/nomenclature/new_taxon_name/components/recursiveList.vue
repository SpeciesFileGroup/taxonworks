<template>
	<ul class="tree-status">
		<li v-for="item, key in objectList" v-if="item.hasOwnProperty(display)">
			...
			<recursive-list :getter-list="getterList" :display="display" :modal-mutation-name="modalMutationName" :action-mutation-name="actionMutationName" :objectList="item"></recursive-list>
		</li>
	</ul>
</template>
