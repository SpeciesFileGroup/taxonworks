<template>
  <div id="topics" class="slide-panel slide-left slide-recent" data-panel-position="relative" v-if="active" data-panel-open="true" data-panel-name="topic_list">
    <div class="slide-panel-header flex-separate">Topic list<new-topic/></div>
    <div class="slide-panel-content">
      <div class="slide-panel-category">
        <ul class="slide-panel-category-content">
          <li v-for="item, index in topics" class="slide-panel-category-item" :class="{ selected : (index == selected) }"@click="loadTopic(item,index)"> {{ item.name }}</li>
        </ul>
      </div>
    </div>
  </div>
