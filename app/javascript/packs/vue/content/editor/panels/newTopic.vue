<template>
  <div>
    <button @click="openWindow" class="button button-default normal-input">New</button>
    <modal v-if="showModal" @close="showModal = false">
      ...
      <form id="new-topic" action="" slot="body">
        ...
        <div class="field">
          <textarea v-model="topic.controlled_vocabulary_term.definition" placeholder="Definition"/>
        </div>
      </form>
      <div slot="footer" class="flex-separate">
        <input class="button normal-input" type="submit" @click.prevent="createNewTopic" :disabled="((topic.controlled_vocabulary_term.name.length < 2) || (topic.controlled_vocabulary_term.definition.length < 2)) ? true : false" value="Create">
      </div>
    </modal>
  </div>
</template>
