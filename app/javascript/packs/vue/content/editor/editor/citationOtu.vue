<template>
  <div :class="{ disabled : disabled }">
    <div class="flex-wrap-column middle menu-button" @click="showModal = citations.length > 0"><span data-icon="citation" class="big-icon"/><span class="tiny_space">OTU Citation</span></div>
    <modal v-if="showModal" @close="showModal = false">
      <h3 slot="header">Citation OTU</h3>
      <ul slot="body">
        <li v-for="item in citations"><span @click="pinItem(item)"/><span v-html="item.object_tag"/></li>
      </ul>
    </modal>
  </div>
</template>
