<template>
  <div :class="{ disabled : !content || contents.length < 1}" >
    <div class="item flex-wrap-column middle menu-button" @click="showModal = contents.length > 0" ><span data-icon="compare" class="big-icon"/><span class="tiny_space">Compare</span></div>
    <modal v-if="showModal" id="compare-modal" @close="showModal = false">
      ...
      <ul slot="body" class="no_bullets">
        <li v-for="item in contents" @click="compareContent(item)"><span data-icon="show"><div class="clone-content-text">{{ item.text }}</div></span><span v-html="item.topic.object_tag + ' - ' + item.otu.object_tag"/></li>
      </ul>
    </modal>
  </div>
</template>
