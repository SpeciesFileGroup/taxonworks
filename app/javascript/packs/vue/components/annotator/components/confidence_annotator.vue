  ...
  <div class="confidence_annotator">
    <div class="switch-radio separate-bottom" v-if="preferences">
      <template v-for="item, index in tabOptions">
        <template v-if="item == 'new' || preferences[item].length && preferences[item].find(keyword => { return !confidenceAlreadyCreated(keyword) })">
          <input
            v-model="view"
            :value="item"
    ...
    <template v-if="preferences && view != 'new' && view != 'all'">
      <div class="field separate-bottom">
        <template v-for="keyword in preferences[view]">
          <button v-if="!confidenceAlreadyCreated(keyword)" @click="createNewWithId(keyword.id)" type="button" class="normal-input button-submit tag_button"> {{ keyword.name }} </button>
        </template>
      </div>
    </template>
    ...
    <template v-if="view == 'new'">
      <autocomplete
        ...
        param="term"/>
      <textarea class="separate-bottom" placeholder="Definition... (minimum is 20 characters)" v-model="confidence.confidence_level_attributes.definition"/>
      <div>
...
export default {
  ...
  methods: {
    ...
    confidenceAlreadyCreated (confidence) {
      return this.list.find(item => { return confidence.id == item.confidence_level_id })
    }
  }
}
</script>
