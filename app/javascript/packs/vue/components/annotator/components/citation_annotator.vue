    ...
    <div v-if="!citation.hasOwnProperty('id')">
      ...
      <div class="separate-bottom inline">
        <autocomplete
          ...
          placeholder="Topic"
          param="term"/>
        <input type="text" class="normal-input inline pages" v-model="topic.pages" placeholder="Pages">
      </div>
      <div class="separate-bottom">
        <button class="button button-submit normal-input" :disabled="!validateFields" @click="createNew()" type="button">Create</button>
      </div>
      <display-list :edit="true" @edit="citation = loadCitation($event)" label="object_tag" :list="list" @delete="removeItem" class="list"/>
    </div>
    <div v-else>
      <span v-if="citation.hasOwnProperty('id')" v-html="citation.object_tag"/>
      ...
      <div class="separate-bottom">
        ...
        <button class="button button-default normal-input" :disabled="!validateFields" @click="citation = newCitation()" type="button">New</button>
      </div>
      <display-list :label="['topic', 'object_tag']" :list="citation.citation_topics" @delete="deleteTopic" class="list"/>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    updateCitation () {
      if (this.topic.topic_id) { this.citation.citation_topics_attributes.push(this.topic) }
