    ...
    <ul class="table-entrys-list">
      <draggable v-model="roles_attributes" @end="onSortable">
        <li class="flex-separate middle" v-for="role, index in roles_attributes" v-if="!role.hasOwnProperty('_destroy')">
          <a :href="getUrl(role)" target="_blank" v-html="getLabel(role)"/>
          <span class="circle-button btn-delete" @click="removePerson(index)"/>
        </li>
      </draggable>
    </ul>
  </div>
...
export default {
  ...
  methods: {
    getUrl (role) {
      if (role.hasOwnProperty('person')) { return `/people/${role.person.id}/edit` } else { return '#' }
    },
