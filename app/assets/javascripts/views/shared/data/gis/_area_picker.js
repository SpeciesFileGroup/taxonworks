...
TW.views.areas = TW.views.areas || {};
TW.views.areas.area_picker = TW.views.areas.area_picker || {};

Object.assign(TW.views.areas.area_picker, {
  ...
  initialize_autocomplete: function (form) {
    ...
    autocomplete_input.autocomplete({
      ...
      open: function (event, ui) {
        TW.views.areas.area_picker.bind_hover(form);
      },
      select: function (event, ui) {    // execute on select event in search text box
        TW.views.areas.area_picker.insert_existing_keyword(form, ui.item.object_id, ui.item.label);
        TW.views.areas.area_picker.clear_keyword_picker(form);
        TW.views.areas.area_picker.make_list_sortable(form);     // was this inadvertantly lost?
        return false;
      }
    ...
    list_items.sortable({
      change: function (event, ui) {
        if ($('form[id^="new_"]').length == 0) {
          TW.views.areas.area_picker.warn_for_save(form.find(".area_picker_message"));
        }
      }
    });
...
// bind a hover event to an ellipsis
  ...
  insert_existing_keyword: function (form, keyword_id, label) {
    ...
    area_list.append($('<input hidden name="' + base_class + '[areas_attributes][' + random_index + '][keyword_id]" value="' + keyword_id + '" >'));

    // insert visible list item
    area_list.append($('<li class="area_item" data-area-index="' + random_index + '">').append(label).append('&nbsp;').append(TW.views.areas.area_picker.remove_link()));
  },
...
//
  bind_new_link: function (form) {
    ...
    form.find("#keyword_picker_add_new").click(function () {
      TW.views.areas.area_picker.insert_new_area(form);
  ...
  insert_new_area: function (form) {
    var base_class = 'areagable_object';
    var random_index = new Date().getTime();
    var area_base = base_class + '[areas_attributes][' + random_index + '][keyword_attributes]';
    var area_list = form.find(".area_list");
    ...
    area_list.append($('<li class="area_item" data-new-area="true" data-area-index="' + random_index + '" >')
      .append(name + "&nbsp;")
      .append(TW.views.areas.area_picker.remove_link())
  ...
  remove_link: function () {
    ...
    TW.views.areas.area_picker.bind_remove_links(link);
    return link;
  },
  ...
  bind_remove_links: function (links) {
    links.click(function () {
      ...
      if (area_id != undefined) {
        ...
        TW.views.areas.area_picker.warn_for_save(area_list.siblings('.area_picker_message'));
        // }
      }
      list_item.remove();
    });
...
_initialize_area_picker_widget = function
  ...
  $('.area_picker').each(function () {
    TW.views.areas.area_picker.initialize_area_picker($(this));
  });
};
