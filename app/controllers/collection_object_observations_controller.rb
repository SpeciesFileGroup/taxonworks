class CollectionObjectObservationsController < ApplicationController
  ...
  def index
    @recent_objects = CollectionObjectObservation.recent_from_project_id($project_id).order(updated_at: :desc).limit(10)
  ...
  def list
    @collection_object_observations = CollectionObjectObservation.with_project_id($project_id).order(:id).page(params[:page]) #.per(10) 
  end
  ...
  def download
    send_data CollectionObjectObservation.generate_download( CollectionObjectObservation.where(project_id: $project_id) ), type: 'text', filename: "collection_object_observations_#{DateTime.now.to_s}.csv"
  end
