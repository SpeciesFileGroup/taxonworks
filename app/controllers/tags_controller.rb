...
class TagsController < ApplicationController
  ...
  def batch_create
    if Tag.batch_create(params.permit(:keyword_id, :object_type, :object_ids).to_h.merge(user_id: sessions_current_user_id, project_id: sessions_current_project_id))
      render json: {success: true}
    else
      render json: {success: false}
    end
