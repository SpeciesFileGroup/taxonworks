class Tasks::CollectionObjects::BrowseController < ApplicationController
  ...
  def set_collection_object
    ...
    @collection_object = CollectionObject.with_project_id(sessions_current_project_id).preload(:identifiers, :collecting_event, depictions: [:image], taxon_determinations: [:determiners]).find(id)
  end
