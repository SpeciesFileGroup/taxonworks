class Tasks::Gis::MatchGeoreferenceController < ApplicationController
  ...
  def filtered_collecting_events
    message = ''
    @collecting_events = CollectingEvent.filter(params).with_project_id(sessions_current_project_id)
  ...
  def filtered_georeferences
    message = ''
    @georeferences = Georeference.filter(params).with_project_id(sessions_current_project_id)
