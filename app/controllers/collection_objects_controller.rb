class CollectionObjectsController < ApplicationController
  ...
  def preview_specimens_batch_load 
    if params[:file] 
      @result = BatchLoad::Import::CollectionObjects::SpecimensInterpreter.new(batch_params)
      digest_cookie(params[:file].tempfile, :Specimens_collection_objects_md5)
      render 'collection_objects/batch_load/specimens/preview'
    else
      flash[:notice] = "No file provided!"
      redirect_to action: :batch_load 
    end
  ...
  # TODO: AUTOGENERATED STUB, check and update
  def create_specimens_batch_load
    if params[:file] && digested_cookie_exists?(params[:file].tempfile, :Specimens_collection_objects_md5)
      @result = BatchLoad::Import::CollectionObjects::SpecimensInterpreter.new(batch_params)
      if @result.create
        flash[:notice] = "Successfully proccessed file, #{@result.total_records_created} TODO RECORD TYPES were created."
        render 'collection_objects/batch_load/specimens/create' and return
      else
        flash[:alert] = 'Batch import failed.'
      end
    else
