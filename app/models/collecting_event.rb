...
=end
  ...
  def to_geo_json_feature
    ...
    geometry = JSON.parse(GeographicItem.connection.select_all("select ST_AsGeoJSON(#{geo_item.geo_type.to_s}::geometry) geo_json from geographic_items where id=#{geo_item.id};")[0]['geo_json'])
    retval   = {
      'type'       => 'Feature',
      'geometry'   => geometry,
      'properties' => {
