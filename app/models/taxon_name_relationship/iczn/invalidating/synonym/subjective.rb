class TaxonNameRelationship::Iczn::Invalidating::Synonym::Subjective < TaxonNameRelationship::Iczn::Invalidating::Synonym
  ...
  def sv_validate_seniority
    if self.subject_taxon_name.cached_nomenclature_date == self.object_taxon_name.cached_nomenclature_date && subject_invalid_statuses.empty?
      r1 = self.subject_taxon_name.original_combination_relationships.reload.sort{|a,b| ORIGINAL_COMBINATION_RANKS.values.index(a.type) <=> ORIGINAL_COMBINATION_RANKS.values.index(b.type) }
      r2 = self.object_taxon_name.original_combination_relationships.reload.sort{|a,b| ORIGINAL_COMBINATION_RANKS.values.index(a.type) <=> ORIGINAL_COMBINATION_RANKS.values.index(b.type) }
      if !r1.empty? && !r2.empty? && ORIGINAL_COMBINATION_RANKS.values.index(r1.last.type) < ORIGINAL_COMBINATION_RANKS.values.index(r2.last.type) && TaxonNameRelationship.where_subject_is_taxon_name(self.subject_taxon_name).with_two_type_bases('TaxonNameRelationship::Iczn::Invalidating::Homonym', 'TaxonNameRelationship::Iczn::Validating').not_self(self).empty?
