    ...
    rv += images.collect do |i|
      image_dwc_array =
        Export::CSV::Dwc::Extension::Media::HEADERS.collect do |h|
          dwc_accessor = DWC_MEDIA_EXTENSION_MAP[h.to_sym]
          dwc_accessor.present? && respond_to?(dwc_accessor) ?
            send(dwc_accessor, i) : nil
        end

      image_fields_hash = i.darwin_core_media_extension_rows
