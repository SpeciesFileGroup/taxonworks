module IconsHelper
  ...
  ICONS = {
    ...
    'batch'         => '<path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/>',
    ...
    'favorite-star' => '<polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>',
    ...
    'hamburger'     => '<path d="M4 5h16"/><path d="M4 12h16"/><path d="M4 19h16"/>',
    'help'          => '<path class="st0" d="M6.8,6.7c0-1.5,0.6-2.7,1.6-3.5c1-0.7,2.2-1.1,3.6-1.1c1.7,0,3,0.5,3.9,1.4c0.9,0.9,1.4,2.1,1.4,3.5	c0,1.1-0.2,2-0.7,2.7c-0.5,0.7-1.2,1.5-2.2,2.3c-0.8,0.7-1.4,1.2-1.7,1.7s-0.5,1.1-0.6,1.7"/><circle cx="12" cy="21.3" r="0.5"/>',
    ...
    'newspaper'     => '<path d="M15 18h-5"/><path d="M18 14h-8"/><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2Zm0 0a2 2 0 0 1-2-2v-9c0-1.1.9-2 2-2h2"/><rect width="8" height="4" x="10" y="6" rx="1"/>',
    ...
    'show'          => '<path d="M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0"/><circle cx="12" cy="12" r="3"/>',
    'summary'       => '<path d="M15 4H7"/><path d="m18 16 3 3-3 3"/><path d="M3 4v13a2 2 0 0 0 2 2h16"/><path d="M7 14h7"/><path d="M7 9h12"/>',
    'square-pen'    => '<path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/>',
  ...
  def icon(name, size: 16, **overrides)
    entry = ICONS[name.to_s]
    paths, custom = entry.is_a?(Hash) ? [entry[:paths], entry.except(:paths)] : [entry, {}]
    attributes = ICON_DEFAULTS.merge(custom, overrides, width: size, height: size)
    content_tag(:svg, paths.to_s.html_safe, attributes)
  end
