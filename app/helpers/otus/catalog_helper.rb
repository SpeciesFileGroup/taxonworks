module Otus::CatalogHelper
  ...
  def otu_descendants_and_synonyms(otu = self, data = {}, similar_otus = [])
    ...
    synonyms = otu.taxon_name&.synonyms.where.not(id: otu.taxon_name.id)&.order(:cached, :cached_author_year)
    ...
    data = { otu_id: otu.id,
             ...
             nomenclatural_synonyms: ( (synonyms&.collect{|l| full_original_taxon_name_label(l) || taxon_name_label(l) } || []) - [a]).uniq, # This is labels, combinations can duplicate
             descendants: []}
