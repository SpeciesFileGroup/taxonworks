if params[:ancestors] != 'false'
  json.ancestors @taxon_name.ancestor_protonyms do |ancestor|
    json.extract! ancestor, :id, :cached_html, :cached_is_valid, :cached_author_year
      json.valid_descendants ancestor.descendants.that_is_valid.count
      json.invalid_descendants ancestor.descendants.that_is_invalid.count
  end
end

json.taxon_name do 
  json.extract! @taxon_name, :id, :cached_html, :cached_is_valid, :cached_author_year
  json.valid_descendants @taxon_name.descendants.that_is_valid.count
  json.invalid_descendants @taxon_name.descendants.that_is_invalid.count
...
json.children @taxon_name.children.order(:name).where(type: 'Protonym').sort_by{|a| [RANKS.index(a.rank_string), a.cached, a.cached_author_year || '']} do |child|
  ...
  json.valid_descendants child.descendants.that_is_valid.count
  json.invalid_descendants child.descendants.that_is_invalid.count
