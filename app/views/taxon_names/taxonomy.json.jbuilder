...
json.children @taxon_name.children.order(:name).where(type: 'Protonym').select { |p| p.id == p.cached_valid_taxon_name_id }.sort_by{|a| [RANKS.index(a.rank_string), a.cached, a.cached_author_year || '']} do |child|
