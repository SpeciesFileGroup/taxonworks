...
json.metadata do
  if @project_id.nil?
    ...
    json.set!("Past week: New project sources", ProjectSource.where('created_at > ? AND project_id = ?', 1.week.ago, @project_id).count)
    json.set!("Past week: New taxon names", TaxonName.where(project_id: @project_id).where('created_at > ?', 1.week.ago).count)
    json.set!("Past week: New collection objects", CollectionObject.where(project_id: @project_id).where('created_at > ?', 1.week.ago).count)
    json.set!("Past week: New collecting events", CollectingEvent.where(project_id: @project_id).where('created_at > ?', 1.week.ago).count)
    json.set!("Past week: New observations", Observation.where(project_id: @project_id).where('created_at > ?', 1.week.ago).count)
    json.set!("Past week: New citations", Citation.where(project_id: @project_id).where('created_at > ?', 1.week.ago).count)
    json.set!("Past week: New citations", Image.where(project_id: @project_id).where('created_at > ?', 1.week.ago).count)
    json.set!("Past week: New roles", Role.where(project_id: @project_id).where('created_at > ?', 1.week.ago).count)
  end
