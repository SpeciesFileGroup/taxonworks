...
RANKS = ( ['NomenclaturalRank'] + ICN + ICZN ).freeze
...
RANKS_LOOKUP = ICN_LOOKUP.invert.merge(ICZN_LOOKUP.invert).freeze

# An Array of Arrays, used in options for select
#   [["class (ICN)", "NomenclaturalRank::Icn::HigherClassificationGroup::ClassRank"] .. ]
RANKS_SELECT_OPTIONS = RANKS_LOOKUP.collect{|k,v| ["#{v} " + ((k.to_s =~ /Iczn/) ? '(ICZN)' : '(ICN)' ), k, {class: ((k.to_s =~ /Iczn/) ? :iczn : :icn) }] }.sort{|a, b| a[0] <=> b[0]}.freeze 
...
FAMILY_RANK_NAMES =  ( FAMILY_RANK_NAMES_ICZN + FAMILY_RANK_NAMES_ICN ).freeze
...
FAMILY_AND_ABOVE_RANK_NAMES = FAMILY_AND_ABOVE_RANK_NAMES_ICZN +
  (NomenclaturalRank::Icn::HigherClassificationGroup.descendants +
   NomenclaturalRank::Icn::FamilyGroup.descendants).collect{|i| i.to_s}.freeze
...
GENUS_RANK_NAMES = ( GENUS_RANK_NAMES_ICZN + GENUS_RANK_NAMES_ICN ).freeze

# All assignable ranks for species groups, for both ICN and ICZN
SPECIES_RANK_NAMES = ( SPECIES_RANK_NAMES_ICZN + SPECIES_RANK_NAMES_ICN ).freeze
