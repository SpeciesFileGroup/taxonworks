class Catalog::Inventory < ::Catalog
  ...
  # @return [Hash{ Integer => Array of Hash }]
  #   Keyed by coordinate OTU id (the base_object of each item).
  #   Each value is an array of hashes, one per unique (type, source) pair
  #   for that OTU, with topics unioned across all items of that pair.
  #   { otu_id => [ { type: String, source: Source, pages: String,
  #                   is_original: Boolean, topics: [Topic] }, ... ] }
  def citations_summary
    items.group_by { |item| item.base_object.id }
      .transform_values do |otu_items|
        otu_items.group_by { |item| [item.object.class.name, item.citation&.source_id] }
