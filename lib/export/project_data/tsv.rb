module ::Export::ProjectData::Tsv
  ...
  def self.generate_dump(project, file)
    ...
    tables = (ActiveRecord::Base.connection.tables - Export::Project::SPECIAL_TABLES - Export::Project::HIERARCHIES.map(&:first)).sort
    ...
    Export::Project::HIERARCHIES.each { |p| dump_hierarchy_table(p, file, project.id) }
    ...
    Tempfile.create do |file|
      buffer = ::Zip::OutputStream.write_buffer(file) do |zipfile|
        ApplicationEnumeration.project_data_classes.each do |k|
          ...
          next if !q.any?
          filename = Zaru::sanitize!("#{k.name}_#{DateTime.now}.tsv")
  ...
  def self.copy_table(klass, target_io, query)
    Tempfile.create do |table_file|
      ...
      target_io.write table_file.read
      table_file.close
    end
  end

  def self.download_async(target_project)
    DownloadProjectTsvJob.perform_later(target_project, stub_download(target_project))
    download
  end
