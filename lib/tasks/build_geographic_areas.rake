namespace :tw do
  namespace :init do
    ...
    desc "Report on the shapefiles.\n
           rake  tw:init:report_on_shapefiles data_directory=/Users/matt/src/sf/tw/gaz/ ?index=[something]"
    task :report_on_shapefiles => [:environment, :data_directory] do
    ...
    desc "Generate the GeographicAreas corresponding to input shape data in SFGs /gaz/ repository.  The input shape data are postgis readable shapefiles.\n 
          ...
          Call:\n
            tw:init:build_geographic_areas data_directory=/Users/matt/src/sf/tw/gaz/ user_id=1 divisions=false NO_GEO_NESTING=1;NO_GEO_VALID=1"
    task :build_geographic_areas => [:environment, :data_directory, :user_id, :build_geographic_area_types] do
      ...
      @divisions ||= true
      # @builder = User.find($user_id)  # When $user_id is set (see :use_id task) this is unnecessary 
      ...
      a = Benchmark.measure { read_dbf(Dir.glob(@args[:data_directory] + '**/*.dbf') + (Dir.glob(@args[:data_directory] + '**/*.txt'))) }
      puts "GeographicAreas built: #{a.strip}."
    end

    desc "Generate the GeographicAreaTypes table, storing results in an index @gat_list."
    task :build_geographic_area_types => [:environment, :data_directory, :user_id] do
      @gat_list = {}
    ...
    def read_dbf(filenames)
      ...
      if earth.count == 0
        ...
        earth.save!
      else # use the (first) record we found earth = earth.first
      end
...
=end
    ...
    #noinspection RubyStringKeysInHashInspection
    # This does not update the database.  See
    #   
