...
### rake tw:project_import:access3i_curculionidae:import_all_curculionidae data_directory=/Users/proceps/src/sf/import/3i/TXT_Curculionidae/ no_transaction=true
...
namespace :tw do
  namespace :project_import do
    namespace :access3i_curculionidae do

      @import_name = 'Curculionidae'

      # A utility class to index data.
      class ImportedData3iCurculionidae
      ...
      task import_all_curculionidae: [:data_directory, :environment] do |t|
        ...
        @relationship_classes = {
            ...
            18 => 'TaxonNameRelationship::Iczn::Invalidating::Synonym::Objective::UnnecessaryReplacementName', ### type designation => relationship source.
            19 => '', ### neotype designation => atribute
            20 => 'TaxonNameRelationship::Iczn::Invalidating::Usage::IncorrectOriginalSpelling', ### lectotype designation => atribute
        ...
        @locality_columns_3iCurculionidae = %w{
          Country
          State
          County
          GLocality
        ...
        if ENV['no_transaction']
          ...
          main_build_loop_3iCurculionidae
        else
      ...
      def main_build_loop_3iCurculionidae
        print "\nStart time: #{Time.now}\n"

        @import = Import.find_or_create_by(name: @import_name)
        @import.metadata ||= {}
        @data =  ImportedData3iCurculionidae.new
        puts @args
        Utilities::Files.lines_per_file(Dir["#{@args[:data_directory]}/**/*.txt"])

        handle_projects_and_users_3iCurculionidae
        #        Current.project_id = 1
        #        $user_id = 1
        raise 'Current.project_id or $user_id not set.'  if Current.project_id.nil? || Current.user_id.nil?
        #        @root = Protonym.find_or_create_by(name: 'Root', rank_class: 'NomenclaturalRank', project_id: Current.project_id) if @root.blank?
        handle_controlled_vocabulary_3iCurculionidae
        # handle_transl_3iCurculionidae
        ...
        #handle_chartable_3iCurculionidae
        #handle_content_types_3iCurculionidae
        #handle_contents_3iCurculionidae

=begin
        $user_id = @trivellone.id
        handle_trivellone_references_3iCurculionidae
        handle_trivellone_unique_species_3iCurculionidae
        handle_trivellone_phytoplasma_group_3iCurculionidae
        handle_trivellone_phytoplasma_taxonomy_3iCurculionidae
        handle_trivellone_phytoplasma_otu_3iCurculionidae
        handle_trivellone_insect_phytoplasma_3iCurculionidae
        handle_trivellone_plant_phytoplasma_3iCurculionidae
        $user_id = @proceps.id
=end

        handle_localities_3iCurculionidae
        #index_collecting_events_from_accessions_new_3iCurculionidae
        soft_validations_3iCurculionidae
        print "\n\n !! Success. End time: #{Time.now} \n\n"
      end

      def handle_projects_and_users_3iCurculionidae
        print "\nHandling projects and users "
        email = 'arboridia@gmail.com'
        project_name = '3i Curculionidae' +  Time.now.to_s
        email1 = 'n.yunakov@gmail.com'
        user_name = 'Nikolai Yunakov'
        @proceps = User.where(email: email).first

        user = User.where(email: email1).first
        if user.nil?
          pwd = rand(36**10).to_s(36)
          user = User.create(email: email1, password: pwd, password_confirmation: pwd, name: user_name, is_administrator: true, self_created: true, is_flagged_for_password_reset: true)
        end
        ...
        pm1 = ProjectMember.create(user: user, project: project, is_project_administrator: true)
        ...
        @data.keywords['3i_imported'] = Keyword.find_or_create_by(name: '3i_imported', definition: 'Imported from 3iCurculionidae database.')
      end
      ...
      def handle_controlled_vocabulary_3iCurculionidae
        ...
        @data.keywords.merge!(
            #'AuthorDrMetcalf' => Predicate.find_or_create_by(name: 'AuthorDrMetcalf', definition: 'Author name from DrMetcalf bibliography database.', project_id: Current.project_id),
            '3i_imported' => Keyword.find_or_create_by(name: '3i_imported', definition: 'Imported from 3i Curculionidae database.', project_id: Current.project_id),
            'introduced' => Keyword.find_or_create_by(name: 'Introduced', definition: 'This species is introduced one.', project_id: Current.project_id),
            'ZeroTotal' => Keyword.find_or_create_by(name: 'ZeroTotal', definition: 'On import there were 0 total specimens recorded in INHS FileMaker database.', project_id: Current.project_id),
            'Allotype' => Keyword.find_or_create_by(name: 'Allotype', definition: 'This specimen is designated as an Allotype', project_id: Current.project_id),
            'CallNumberDrMetcalf' => Predicate.find_or_create_by(name: 'call_number_dr_metcalf', definition: 'Call Number from DrMetcalf bibliography database.', project_id: Current.project_id),
            #'AuthorReference' => Predicate.find_or_create_by(name: 'author_reference', definition: 'Author string as it appears in the nomenclatural reference.', project_id: Current.project_id),
            #'YearReference' => Predicate.find_or_create_by(name: 'year_reference', definition: 'Year string as it appears in the nomenclatural reference.', project_id: Current.project_id),
            #'Ethymology' => Predicate.find_or_create_by(name: 'ethymology', definition: 'Ethymology.', project_id: Current.project_id),
            'TypeDepository' => Predicate.find_or_create_by(name: 'type_depository', definition: 'Type depository collection (text value)', project_id: Current.project_id),
            'HostPlant' => Predicate.find_or_create_by(name: 'host_plant', definition: 'Host plant (text value).', project_id: Current.project_id),
            'YearRem' => Predicate.find_or_create_by(name: 'nomenclatural_string', definition: 'Nomenclatural remarks (if any)', project_id: Current.project_id),
            'Typification' => Predicate.find_or_create_by(name: 'type_designated_by', definition: 'Type designated by (string value).', project_id: Current.project_id),
            'FirstRevisor' => Predicate.find_or_create_by(name: 'first_revisor_action', definition: 'First revisor action (string value)', project_id: Current.project_id),
            'PageAuthor' => Predicate.find_or_create_by(name: 'page_author', definition: 'Page author (string value)', project_id: Current.project_id),
            'SimilarSpecies' => Predicate.find_or_create_by(name: 'similar_species', definition: 'Similar species (string value)', project_id: Current.project_id),
            'license' => Predicate.find_or_create_by(name: 'license', definition: 'License (value from mx database)', project_id: Current.project_id),
            'copyright_holder' => Predicate.find_or_create_by(name: 'copyright_holder', definition: 'Copyright holder (value from mx database)', project_id: Current.project_id),
            'Reference_strain' => Predicate.find_or_create_by(name: 'reference_strain', definition: 'Reference_strain (value from Phytoplasma database)', project_id: Current.project_id),
            'Reference_strain_acronym' => Predicate.find_or_create_by(name: 'reference_strain_acronym', definition: 'Reference_strain_acronym (value from Phytoplasma database)', project_id: Current.project_id),
            'Genbank_number' => Predicate.find_or_create_by(name: 'genbank_number', definition: 'Genbank_number (value from Phytoplasma database)', project_id: Current.project_id),
            'Genbank_link' => Predicate.find_or_create_by(name: 'genbank_link', definition: 'Genbank_link (value from Phytoplasma database)', project_id: Current.project_id),
            '16Sr_group' => Predicate.find_or_create_by(name: '16Sr_group', definition: '16Sr_group (value from Phytoplasma database)', project_id: Current.project_id),
            '16Sr_subgroup' => Predicate.find_or_create_by(name: '16Sr_subgroup', definition: '16Sr_subgroup (value from Phytoplasma database)', project_id: Current.project_id),
            'KeyN' => Namespace.find_or_create_by(institution: '3i Curculionidae', name: '3i_Curculionidae_KeyN_ID', short_name: '3i_Curculionidae_KeyN_ID'),
            'Key3' => Namespace.find_or_create_by(institution: '3i Curculionidae', name: '3i_Curculionidae_Source_ID', short_name: '3i_Curculionidae_Source_ID'),
            'Key1' => Namespace.find_or_create_by(institution: '3i Curculionidae', name: '3i_Curculionidae_Key1_ID', short_name: '3i_Curculionidae_Key1_ID'),
            'Key2' => Namespace.find_or_create_by(institution: '3i Curculionidae', name: '3i_Curculionidae_Key2_ID', short_name: '3i_Curculionidae_Key2_ID'),
            'Key' => Namespace.find_or_create_by(institution: '3i Curculionidae', name: '3i_Curculionidae_Taxon_ID', short_name: '3i_Curculionidae_Taxon_ID'),

            'Taxonomy' => Keyword.find_or_create_by(name: 'Taxonomy updated', definition: 'Taxonomical information entered to the DB.', project_id: Current.project_id),
            'Typhlocybinae' => Keyword.find_or_create_by(name: 'Typhlocybinae updated', definition: 'Information related to Typhlocybinae entered to the DB.', project_id: Current.project_id),
            'Illustrations' => Keyword.find_or_create_by(name: 'Illustrations exported', definition: 'Illustrations of Typhlocybinae species entered to the DB.', project_id: Current.project_id),
            'Distribution' => Keyword.find_or_create_by(name: 'Distribution exported', definition: 'Illustrations on species distribution entered to the DB.', project_id: Current.project_id),
            'REF_phyt_strain' => Keyword.find_or_create_by(name: 'REF_phyt_strain', definition: 'REF_phyt_strain (value from Phytoplasma database).', project_id: Current.project_id),
            'Notes' => Topic.find_or_create_by(name: 'Notes', definition: 'Notes topic on the OTU.', project_id: Current.project_id),
            'Host' => BiologicalProperty.find_or_create_by(name: 'Host', definition: 'An animal or plant on or in which a parasite or commensal organism lives.', project_id: Current.project_id),
            'Pathogen' => BiologicalProperty.find_or_create_by(name: 'Pathogen', definition: 'A bacterium, virus, or other microorganism that can cause disease.', project_id: Current.project_id),
            'Herbivor' => BiologicalProperty.find_or_create_by(name: 'Herbivor', definition: 'An animal that feeds on plants.', project_id: Current.project_id),
            'Parasitoid' => BiologicalProperty.find_or_create_by(name: 'Parasitoid', definition: 'An organism that lives in or on another organism.', project_id: Current.project_id),
            'Attendant' => BiologicalProperty.find_or_create_by(name: 'Attendant', definition: 'An insect attending another insect.', project_id: Current.project_id),
            'Symbiont' => BiologicalProperty.find_or_create_by(name: 'Symbiont', definition: 'An insect leaving togeather with another insect.', project_id: Current.project_id),
        ...
        @data.biocuration_classes.merge!(
            'Specimens' => BiocurationClass.find_or_create_by(name: 'Adult', definition: 'Number of adult specimens.', project_id: Current.project_id),
            'Males' => BiocurationClass.find_or_create_by(name: 'Male', definition: 'Number of male specimens.', project_id: Current.project_id),
            'Females' => BiocurationClass.find_or_create_by(name: 'Female', definition: 'Number of female specimens.', project_id: Current.project_id),
            'Nymphs' => BiocurationClass.find_or_create_by(name: 'Immature', definition: 'Number of immature specimens.', project_id: Current.project_id),
            'Exuvia' => BiocurationClass.find_or_create_by(name: 'Exuvia', definition: 'Number of exuvia specimens.', project_id: Current.project_id)
        )
        ...
        @data.topics.merge!(
            ...
            'Synonymy' => Topic.find_or_create_by(name: 'synonymy', definition: 'Source has synonymy records.', project_id: Current.project_id),
            'Host' => Topic.find_or_create_by(name: 'host plants', definition: 'Source has host plant records.', project_id: Current.project_id),
            'Parasitoids' => Topic.find_or_create_by(name: 'parasitoids', definition: 'Source has parasitoid records.', project_id: Current.project_id),
            'Infection_status_negative' => Topic.find_or_create_by(name: 'Tested negative for Phytoplasma', definition: 'Tested negative for Phytoplasma', project_id: Current.project_id),
            )

        @host_plant_relationship = BiologicalRelationship.where(name: 'feeds on', project_id: Current.project_id).first
        ...
        @parasitoid_relationship = BiologicalRelationship.where(name: 'is parasitized by', project_id: Current.project_id).first
        ...
        @attendance_relationship = BiologicalRelationship.where(name: 'is visitor of', project_id: Current.project_id).first
        ...
        @mutualism_relationship = BiologicalRelationship.where(name: 'is mutualistically associated with', project_id: Current.project_id).first
        ...
        @insect_phytoplasma_relationship = BiologicalRelationship.where(name: 'is vector of', project_id: Current.project_id).first
        ...
        @plant_phytoplasma_relationship = BiologicalRelationship.where(name: 'is infected with', project_id: Current.project_id).first
      ...
      def handle_litauthors_3iCurculionidae
        # Author
        # FullName
      ...
      def handle_references_3iCurculionidae
        ...
        file.each do |row|
          i += 1
          print "\r#{i}"
          journal, serial_id, volume, number, pages = parse_bibliography_3iCurculionidae(row['Bibliography'])
          year, year_suffix = parse_year_3iCurculionidae(row['Year'])
      ...
      def handle_trivellone_references_3iCurculionidae()
        # PK_reference
        # reference
        # authors
        # year
        ...
        file.each do |row|
          ...
          journal, serial_id, volume, number, pages = parse_bibliography_3iCurculionidae(row['journal'])
          year = row['year']
      ...
      def parse_bibliography_3iCurculionidae(bibl)
      ...
      def parse_year_3iCurculionidae(year)
      ...
      def handle_nouns_3iCurculionidae
      ...
      def handle_taxonomy_3iCurculionidae
        ...
        confidence = ConfidenceLevel.find_or_create_by(name: 'Verified', definition: 'Verified against the original source', project_id: Current.project_id).id
        ...
        file.each do |row|
          i += 1
          print "\r#{i}"
          if row['Name'].include?('_incer') || row['Name'].include?(' incer') || row['Name'] == 'Incertae sedis' || row['Name'] == 'Unplaced'
            @data.incertae_sedis[row['Key']] = @data.taxon_index[row['Parent']]
          elsif row['Status'] == '7' ### common name
          elsif row['Status'] == '8' #### && !find_taxon_3iCurculionidae(row['Parent']).try(:rank_class).to_s.include?('Family') ### combination
          elsif row['Status'] == '2'
            ...
            otu.identifiers.create!(type: 'Identifier::Local::Import', namespace: @data.keywords['Key'], identifier: row['Key']) if otu
          elsif false && row['Status'] == '18'
            taxon = find_taxon_3iCurculionidae(row['Parent'])
            ...
            #              taxon.valid? ? taxon.save! : byebug
          elsif row['Status'] == '25'  && !find_taxon_3iCurculionidae(row['Parent']).rank_class.to_s.include?('Family') # emendation
            taxon = find_taxon_3iCurculionidae(row['Parent'])
            ...
            #parent = row['Parent'].blank? ? @root : Protonym.with_identifier('3iCurculionidae_Taxon_ID ' + row['Parent']).find_by(project_id: Current.project_id)
            parent = row['Parent'].blank? ? @root : @data.incertae_sedis[row['Parent']] ? TaxonName.find(@data.incertae_sedis[row['Parent']]) : find_taxon_3iCurculionidae(row['Parent'])
            if row['Rank'].to_i > 5
            ...
            if !row['OriginalSpecies'].blank? && row['Rank'] == '2'
              parent = find_taxon_3iCurculionidae(row['OriginalSpecies'])
              rank = @ranks[1]
            end
            byebug if row['Rank'].blank?

            #rank = row['Rank'] == '0' ? parent.rank_class : @ranks[row['Rank'].to_i]
            source = row['Key3'].blank? ? nil : @data.publications_index[row['Key3']] # Source.with_identifier('3iCurculionidae_Source_ID ' + row['Key3']).first
            #source_id = Identifier.where(cached: '3iCurculionidae_Source_ID ' + row['Key3'].to_s).limit(1).first if source.nil? && !row['Key3'].blank?
            #source = source_id.identifier_object if source.nil? && !source_id.nil?
            if row['Rank'] == '0' || row['Rank'] == '19'
              byebug if parent.parent.nil?
              parent = parent.parent
            end
            name = name.gsub('2222', '').gsub('1111', '').gsub(' (+)', '')
      ...
      def handle_taxon_name_relationships_3iCurculionidae
        ...
        file.each do |row|
          ...
          taxon = nil
          taxon = find_taxon_3iCurculionidae(row['Key'])
          if !taxon.nil? ### original combinations, synonyms, types
            taxon.original_genus = find_taxon_3iCurculionidae(row['OrigGen']) unless row['OrigGen'].blank?
            taxon.original_subgenus = find_taxon_3iCurculionidae(row['OrigSubGen']) unless row['OrigSubGen'].blank?
            taxon.original_species = find_taxon_3iCurculionidae(row['OriginalSpecies']) unless row['OriginalSpecies'].blank?
            taxon.original_subspecies = find_taxon_3iCurculionidae(row['OriginalSubSpecies']) unless row['OriginalSubSpecies'].blank?
            taxon.original_variety = taxon if row['Name'].include?(' var. ')
            taxon.original_form = taxon if row['Name'].include?(' f. ')
            ...
            if taxon.rank_string =~ /Genus/ && !row['Type'].blank?
              ...
              when 'original monotypy'
                taxon.type_species_by_original_original_monotypy = find_taxon_3iCurculionidae(row['Type'])
              when 'monotypy'
                taxon.type_species_by_monotypy = find_taxon_3iCurculionidae(row['Type'])
              when 'subsequent monotypy'
                taxon.type_species_by_subsequent_monotypy = find_taxon_3iCurculionidae(row['Type'])
              when 'original designation'
                taxon.type_species_by_original_designation = find_taxon_3iCurculionidae(row['Type'])
              when 'subsequent designation'
                #taxon.type_species_by_subsequent_designation = find_taxon_3iCurculionidae(row['Type'])
                source1 = row['Key3a'].blank? ? nil : @data.publications_index[row['Key3a']]
                tssd = taxon.related_taxon_name_relationships.create(subject_taxon_name: find_taxon_3iCurculionidae(row['Type']),  type: 'TaxonNameRelationship::Typification::Genus::Subsequent::SubsequentDesignation')
              ...
              when 'ruling by commission'
                taxon.type_species_by_ruling_by_commission = find_taxon_3iCurculionidae(row['Type'])
              else
                taxon.type_species = find_taxon_3iCurculionidae(row['Type'])
              end
            end
            taxon.type_genus = find_taxon_3iCurculionidae(row['Type']) if taxon.rank_string =~ /Family/ && !row['Type'].blank?
            taxon.iczn_set_as_primary_homonym_of = find_taxon_3iCurculionidae(row['Homonym']) if !row['Homonym'].blank? && row['Status'] == '3'
            taxon.iczn_set_as_secondary_homonym_of = find_taxon_3iCurculionidae(row['Homonym']) if !row['Homonym'].blank? && row['Status'] == '4'
            taxon.iczn_set_as_homonym_of = find_taxon_3iCurculionidae(row['Homonym']) if !row['Homonym'].blank? && row['Status'] == '5'
            #taxon.iczn_set_as_replacement_name_of = find_taxon_3iCurculionidae(row['NomenNovumFor']) if !row['NomenNovumFor'].blank?
            taxon.iczn_set_as_misspelling_of = find_taxon_3iCurculionidae(row['MisspellingOf']) if row['OriginalCombinationOf'].blank? && !row['MisspellingOf'].blank?
            taxon.iczn_set_as_misspelling_of = find_taxon_3iCurculionidae(row['Parent']) if row['OriginalCombinationOf'].blank? && row['MisspellingOf'].blank? && row['Status'] == '9'
            taxon.iczn_set_as_incorrect_original_spelling_of = find_taxon_3iCurculionidae(row['OriginalCombinationOf']) if !row['OriginalCombinationOf'].blank? && row['Status'] == '9'
            taxon.iczn_set_as_misapplication_of = find_taxon_3iCurculionidae(row['MisapplicationFor']) if !row['MisapplicationFor'].blank? && row['Status'] == '11'
            ...
            if !@data.emendation[row['Parent']].blank? && row['OriginalCombinationOf'] == row['Parent']
              ...
              #source = find_taxon_3iCurculionidae(@data.emendation[row['Parent']]).try(['Key3']).nil? ? nil : @data.publications_index[@data.emendation[row['Parent']]['Key3']]
              tnr = TaxonNameRelationship::Iczn::PotentiallyValidating::FirstRevisorAction.create(subject_taxon_name: find_taxon_3iCurculionidae(row['Parent']), object_taxon_name: taxon)
            ...
            if !row['NomenNovumFor'].blank?
              ...
              if row['YearRem'].to_s.include?('unneded n.nov.')
                tnr = TaxonNameRelationship.create(subject_taxon_name: taxon, object_taxon_name: find_taxon_3iCurculionidae(row['NomenNovumFor']), type: 'TaxonNameRelationship::Iczn::Invalidating::Synonym::Objective::UnnecessaryReplacementName')
              else
                tnr = TaxonNameRelationship.create(subject_taxon_name: find_taxon_3iCurculionidae(row['NomenNovumFor']), object_taxon_name: taxon, type: 'TaxonNameRelationship::Iczn::Invalidating::Synonym::Objective::ReplacedHomonym')
              end
              if !source.blank? && !tnr.id.nil?
                tnr.citations.create(source_id: source, pages: row['Page'], is_original: true)
              elsif tnr.id.nil?
              ...
              tnr = TaxonNameRelationship.create(subject_taxon_name: taxon, object_taxon_name: find_taxon_3iCurculionidae(row['Parent']), type: @relationship_classes[row['Status'].to_i])
            ...
            if homonym_statuses.include?(row['Status']) && row['Rank'] == '0'
              if TaxonNameRelationship.where_subject_is_taxon_name(taxon.id).with_type_base('TaxonNameRelationship::Iczn::Invalidating::Synonym').first.nil?
                tnr = TaxonNameRelationship.create(subject_taxon_name: taxon, object_taxon_name: find_taxon_3iCurculionidae(row['Parent']), type: 'TaxonNameRelationship::Iczn::Invalidating::Synonym::Subjective')
                byebug if !tnr.nil? && tnr.id.nil?
              end
            end
          elsif row['Status'] == '8' || row['Status'] == '25' ### taxon name combinations
            taxon = find_taxon_3iCurculionidae(row['CombinationOf']) || find_taxon_3iCurculionidae(row['Parent'])
            ...
            origgen = row['OrigGen'].blank? ? nil : find_taxon_3iCurculionidae(row['OrigGen'])
            origsubgen = row['OrigSubGen'].blank? ? nil : find_taxon_3iCurculionidae(row['OrigSubGen'])
            origspecies = row['OriginalSpecies'].blank? ? nil : find_taxon_3iCurculionidae(row['OriginalSpecies'])
            origsubspecies = row['OriginalSubSpecies'].blank? ? nil : find_taxon_3iCurculionidae(row['OriginalSubSpecies'])
            ...
            c.parent = origgen.blank? ? find_taxon_3iCurculionidae(row['Parent']).parent : origgen.parent
            i3_combination = ''
            if taxon.rank_string =~ /Genus/
            ...
            begin
              if c.type == 'Combination' && !i3_combination.blank? && i3_combination != c.cached
                #c.data_attributes.create(type: 'ImportAttribute', import_predicate: 'combination_in_3iCurculionidae', value: i3_combination)
                #c.data_attributes.create(type: 'ImportAttribute', import_predicate: 'name_in_3iCurculionidae', value: row['Name'])
            ...
            CommonName.create!(otu: find_taxon_3iCurculionidae(row['Parent']).otus.first, name: row['Name'], language: lng)
            #          elsif row['Status'] == '13' && row['Rank'] == '0' # Nomen nudum
            #            tnr = TaxonNameRelationship.create(subject_taxon_name: taxon, object_taxon_name: find_taxon_3iCurculionidae(row['Parent']), type: 'TaxonNameRelationship::Iczn::Invalidating')
          elsif row['Status'] == '2' || !row['OriginalCombinationOf'].blank? ### Original combination
            taxon = find_taxon_3iCurculionidae(row['OriginalCombinationOf']) || find_taxon_3iCurculionidae(row['Parent'])
            if taxon.blank?
              byebug
            else
              taxonid = taxon.id
            ...
            taxon.original_species = find_taxon_3iCurculionidae(row['OriginalSpecies']) unless row['OriginalSpecies'].blank?
            taxon.original_subspecies = find_taxon_3iCurculionidae(row['OrigOriginalSubSpecies']) unless row['OriginalSubSpecies'].blank?
            taxon.original_variety = taxon if row['Name'].include?(' var. ')
            taxon.original_form = taxon if row['Name'].include?(' f. ')
            if taxon.rank_string =~ /Species/
              if taxon.original_species.nil?
      ...
      def index_collecting_events_from_accessions_new_3iCurculionidae
        ...
        ac.each do |row|
          ...
          tmp_ce['County'] = geo_translate_3iCurculionidae(tmp_ce['County']) unless tmp_ce['County'].blank?
          tmp_ce['State'] = geo_translate_3iCurculionidae(tmp_ce['State']) unless tmp_ce['State'].blank?
          tmp_ce['Country'] = geo_translate_3iCurculionidae(tmp_ce['Country']) unless tmp_ce['Country'].blank?

          ce = find_or_create_collecting_event_3iCurculionidae(tmp_ce)

        end
      end

      def geo_translate_3iCurculionidae(name)
        GEO_NAME_TRANSLATOR[name] ? GEO_NAME_TRANSLATOR[name] : name
      end

      def handle_host_plant_name_dictionary_3iCurculionidae
        # CommonName
        # HostPlant
        # Phylum
        # Class
...
=begin
          ...
          if otu.nil?
            ...
            unless row['Phylum'].blank?
              plant = Protonym.find_or_create_by!(name: row['Phylum'], rank_class: Ranks.lookup(:icn, 'phylum'), parent: parent, project_id: Current.project_id)
              parent = plant
            end
            unless row['Class'].blank?
              plant = Protonym.find_or_create_by!(name: row['Class'], rank_class: Ranks.lookup(:icn, 'class'), parent: parent, project_id: Current.project_id)
              parent = plant
            end
            unless row['Order'].blank?
              plant = Protonym.find_or_create_by!(name: row['Order'], rank_class: Ranks.lookup(:icn, 'order'), parent: parent, project_id: Current.project_id)
              parent = plant
            end
            unless row['Family'].blank?
              plant = Protonym.find_or_create_by!(name: row['Family'], rank_class: Ranks.lookup(:icn, 'family'), parent: parent, project_id: Current.project_id)
              parent = plant
              otu = Otu.find_or_create_by!(taxon_name_id: plant.id, project_id: Current.project_id).id
              @data.host_plant_index[row['Family']] = otu
            end
            unless row['Genus'].blank?
              plant = Protonym.find_or_create_by!(name: row['Genus'], rank_class: Ranks.lookup(:icn, 'genus'), parent: parent, project_id: Current.project_id)
              parent = plant
            end
            unless row['Subgenus'].blank?
              plant = Protonym.find_or_create_by!(name: row['Subgenus'], rank_class: Ranks.lookup(:icn, 'subgenus'), parent: parent, project_id: Current.project_id)
              parent = plant
            end
            unless row['Species'].blank?
              name = row['Species'].gsub('x ', '')
              if row['Variety'].blank? && !author.blank?
                plant = Protonym.find_or_create_by!(name: name, rank_class: Ranks.lookup(:icn, 'species'), verbatim_author: author, parent: parent, project_id: Current.project_id)
              else
                plant = Protonym.find_or_create_by!(name: name, rank_class: Ranks.lookup(:icn, 'species'), parent: parent, project_id: Current.project_id)
              end
            ...
            unless row['Variety'].blank?
              plant = Protonym.find_or_create_by!(name: row['Variety'], rank_class: Ranks.lookup(:icn, 'variety'), verbatim_author: author, parent: parent, project_id: Current.project_id)
            end
            byebug if plant.try(:id).nil?
            otu = Otu.find_or_create_by!(taxon_name_id: plant.id, project_id: Current.project_id)
=end
          otu = Otu.find_or_create_by!(name: row['HostPlant'], project_id: Current.project_id)
            byebug if otu.nil?
            @data.unique_host_plant_index[row] = otu.id
          #          end

          unless row['CommonName'].blank?
            #lng = Language.find_by_alpha_3_bibliographic(@languages[row['lng'].to_s.downcase])
            c = CommonName.new(otu_id: otu, name: row['CommonName'])
            begin
              c.save
            rescue ActiveRecord::RecordInvalid
              byebug
          ...
          @data.host_plant_index[row['HostPlant']] = otu.id
          @data.host_plant_index[row['CommonName']] = otu.id unless row['CommonName'].blank?

        end
      end

      def handle_host_plants_3iCurculionidae
        # Key
        # Family
        # Name
        # CommonName
        ...
        file.each do |row|
          ...
          object = find_otu_3iCurculionidae(row['key'])
          subject = nil
          ...
          if !subject.nil?
            ...
            subject = Otu.find_or_create_by!(name: row['Name'], project_id: Current.project_id)
          elsif !row['CommonName'].blank?
            subject = Otu.find_or_create_by!(name: row['CommonName'], project_id: Current.project_id)
          elsif !row['Family'].blank?
            subject = Otu.find_or_create_by!(name: row['Family'], project_id: Current.project_id)
          end

          s = find_publication_id_3iCurculionidae(row['Key3'])
      ...
      def handle_citation_topics_3iCurculionidae
        # Key3
        # Key
        # Descriptions
        # Records
        ...
        file.each do |row|
          ...
          p = find_taxon_3iCurculionidae(row['Key'])
          o = find_otu_3iCurculionidae(row['Key'])
          next if o.nil?
          if p.nil?
            ...
            byebug if p.nil?
            c = o.citations.find_or_create_by!(source_id: source, project_id: Current.project_id) unless o.nil?
            #c = p.citations.find_or_create_by!(source_id: source, project_id: Current.project_id)
            #

            if row['Descriptions'] == '1' && row['Types'] == '1'
              c.citation_topics.find_or_create_by(topic: @data.topics['Types'], project_id: Current.project_id)
              p.confidences.create(position: 1, confidence_level_id: confidence)
            end
            c.citation_topics.find_or_create_by(topic: @data.topics['Descriptions'], project_id: Current.project_id) if row['Descriptions'] == '1' && row['Types'] == '0'
            c.citation_topics.find_or_create_by(topic: @data.topics['Records'], project_id: Current.project_id) if row['Records'] == '1'
            c.citation_topics.find_or_create_by(topic: @data.topics['Pictures'], project_id: Current.project_id) if row['Pictures'] == '1'
            c.citation_topics.find_or_create_by(topic: @data.topics['Keys'], project_id: Current.project_id) if row['Keys'] == '1'
            c.citation_topics.find_or_create_by(topic: @data.topics['Synonymy'], project_id: Current.project_id) if row['Synonymy'] == '1'
            c.citation_topics.find_or_create_by(topic: @data.topics['Host'], project_id: Current.project_id) if row['Host'] == '1'
            ...
            if row['Synonymy'] == '1'
              ...
              unless tr.nil?
                cit = tr.citations.find_or_create_by!(source_id: source, project_id: Current.project_id)
      ...
      def handle_distribution_3iCurculionidae
        handle_countries_3iCurculionidae
        ...
        file.each do |row|
          i += 1
          print "\r#{i}"
          otu = find_otu_3iCurculionidae(row['Key'])
          source = find_publication_id_3iCurculionidae(row['Key3'])
          ...
          if !otu.nil? && !source.nil? && !ga.nil?
            ad = AssertedDistribution.find_or_create_by(
                ...
                project_id: Current.project_id )
            c = ad.citations.new(source_id: source, project_id: Current.project_id)
            ad.save
      ...
      def handle_countries_3iCurculionidae
        #Key6
        # Count
        # Country
        # LongCentr
      ...
      def handle_museums_3iCurculionidae
        # Abbreviation
        # Museum
        # Country
        # Location
      ...
      def handle_localities_3iCurculionidae
        handle_museums_3iCurculionidae
        ...
        file.each do |row|
          i += 1
          print "\r#{i}"
          collecting_event = find_or_create_collecting_event_3iCurculionidae(row)
          repository = nil
          ...
          count_fields.each do |count|
            unless row[count].blank?
              ...
              begin
                specimen.save!
                Citation.find_or_create_by!(citation_object: specimen, source_id: source, project_id: Current.project_id) unless source.blank?
                objects += [specimen]
                ...
                #                unless host.blank?
                #                  identifier = Identifier.where(namespace_id: @taxon_namespace.id, identifier: host, project_id: Current.project_id)
                ...
                add_bioculation_class_3iCurculionidae(specimen, count)
              rescue ActiveRecord::RecordInvalid
                byebug
              end
            end
          end
          add_identifiers_3iCurculionidae(objects, row)
          add_determinations_3iCurculionidae(objects, row)
        end
      end
      ...
      def find_or_create_collecting_event_3iCurculionidae(ce)
        ...
        latitude, longitude = parse_lat_long_3iCurculionidae(ce) unless [4, 5, 6].include?(ce['Precision'].to_i)
        sdm, sdd, sdy, edm, edd, edy = parse_dates_3iCurculionidae(ce)
        elevation, verbatim_elevation = parse_elevation_3iCurculionidae(ce)
        geographic_area = ce['TW_id'].blank? ? nil : GeographicArea.find(ce['TW_id'])
        geolocation_uncertainty = parse_geolocation_uncertainty_3iCurculionidae(ce)
      ...
      def parse_geolocation_uncertainty_3iCurculionidae(ce)
      ...
      def add_bioculation_class_3iCurculionidae(o, bcc)
      ...
      def add_identifiers_3iCurculionidae(objects, row)
      ...
      def add_determinations_3iCurculionidae(objects, row)
        otu = find_otu_3iCurculionidae(row['Key'])
        ...
        objects.each do |o|
          unless otu.nil?
            ...
            if !row['Type'].blank?
              type = @type_type_3iCurculionidae[row['Type'].downcase]
              unless type.nil?
      ...
      def parse_lat_long_3iCurculionidae(ce)
        latitude, longitude = nil, nil
      ...
      def parse_dates_3iCurculionidae(ce)
        # Date
        # DateTo
      ...
      def parse_elevation_3iCurculionidae(ce)
        # ElevationM
        # ElevationF
        ft =  ce['ElevationF']
        m = ce['ElevationM']
      ...
      def find_taxon_id_3iCurculionidae(key)
        #@data.taxon_index[key.to_s] || Protonym.with_identifier('3iCurculionidae_Taxon_ID ' + key.to_s).find_by(project_id: Current.project_id).try(:id)
        @data.taxon_index[key.to_s] || Identifier.where(cached: '3i_Curculionidae_Taxon_ID ' + key.to_s, identifier_object_type: 'TaxonName', project_id: Current.project_id).limit(1).pluck(:identifier_object_id).first
      end

      def find_taxon_3iCurculionidae(key)
        Identifier.find_by(cached: '3i_Curculionidae_Taxon_ID ' + key.to_s, identifier_object_type: 'TaxonName', project_id: Current.project_id).try(:identifier_object)
        #Protonym.with_identifier('3iCurculionidae_Taxon_ID ' + key.to_s).find_by(project_id: Current.project_id)
      end

      def find_otu_3iCurculionidae(key)
        otu = nil
        # otu = Otu.joins(taxon_name: :identifiers).where(identifiers: {cached: '3iCurculionidae_Taxon_ID ' + key.to_s}, project_id: Current.project_id).first
        # otu = Otu.joins(:identifiers).where(identifiers: {cached: '3iCurculionidae_Taxon_ID ' + key.to_s}, project_id: Current.project_id).first if otu.nil?

        r = Identifier.find_by(cached: '3i_Curculionidae_Taxon_ID ' + key.to_s, project_id: Current.project_id)
        return nil if r.nil?
      ...
      def find_publication_id_3iCurculionidae(key3)
        @data.publications_index[key3.to_s] || Identifier.where(cached: '3i_Curculionidae_Source_ID ' + key3.to_s).limit(1).pluck(:identifier_object_id).first
      end

      def find_publication_3iCurculionidae(key3)
        @data.publications_index[key3.to_s] || Identifier.where(cached: '3i_Curculionidae_Source_ID ' + key3.to_s).limit(1).first
      end
      ...
      def handle_parasitoids_3iCurculionidae
        ...
        file.each do |row|
          ...
          print "\r#{i}"
          p = Protonym.find_by(name: row['Order'], rank_class: Ranks.lookup(:iczn, 'Order'), project_id: Current.project_id)
          p = Protonym.find_or_create_by(name: row['Superfamily'], parent_id: p.id, rank_class: Ranks.lookup(:iczn, 'Superfamily'), project_id: Current.project_id) unless row['Superfamily'].blank?
          family = Protonym.find_or_create_by(name: row['Family'], rank_class: Ranks.lookup(:iczn, 'Family'), project_id: Current.project_id) unless row['Family'].blank?
          if family.parent_id != p.id
            family.parent_id = p.id
            family.save!
          end
          p = family
          p = Protonym.find_or_create_by(name: row['Genus'], parent_id: p.id, rank_class: Ranks.lookup(:iczn, 'Genus'), project_id: Current.project_id) unless row['Genus'].blank?
          p = Protonym.find_or_create_by(name: row['Species'], parent_id: p.id, rank_class: Ranks.lookup(:iczn, 'Species'), project_id: Current.project_id) if !row['Species'].blank? && row['Species'].to_s.length > 1
          ...
          parasitoid = Otu.find_or_create_by(taxon_name_id: p.id, project_id: Current.project_id)
          taxon = find_otu_3iCurculionidae(row['Key'])
          source = find_publication_id_3iCurculionidae(row['Key3'])
          if parasitoid && taxon
            ...
            Citation.find_or_create_by!(citation_object: ba, source_id: source, project_id: Current.project_id, pages: row['Page']) unless source.blank?
          else
            print "\nRow #{row} is problematic\n"
          end
        end
      ...
      def handle_transl_3iCurculionidae
        # Flag
        # KeyN
        # Num
        # Abr
        ...
        file.each_with_index do |row, i|
          ...
          if row['Abr'] == 'En'
            ...
            matrix = Identifier.where(cached: '3iCurculionidae_KeyN_ID ' + row['KeyN']).limit(1).first.identifier_object
      ...
      def handle_characters_3iCurculionidae
        handle_morph_3iCurculionidae
        # Key1
        # # KeyN
        # # Char
        # # Type
        ...
        file.each do |row|
          ...
          descriptor = Descriptor.create!(name: row['CharEn'].empty? ? '?' : char_name_3iCurculionidae(row['CharEn']).capitalize, short_name: row['CharEn'].empty? ? '?' : char_name_3iCurculionidae(row['CharEn']).capitalize, description_name: row['CharEn'].empty? ? '?' : char_for_description_3iCurculionidae(row['CharEn']).capitalize, key_name: row['CharEn'].empty? ? '?' : char_for_key_3iCurculionidae(row['CharEn']).capitalize, type: t, position: row['Char'].to_i + 1 )
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_name_3iCurculionidae(row['CharRu']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'name', language_id: lngru) unless row['CharRu'].nil?
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_name_3iCurculionidae(row['CharSp']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'name', language_id: lngsp) unless row['CharSp'].nil?
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_name_3iCurculionidae(row['CharZh']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'name', language_id: lngzh) unless row['CharZh'].nil?
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_description_3iCurculionidae(row['CharRu']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'description_name', language_id: lngru) if !row['CharRu'].nil? && row['CharRu'] =~ /[\{\[]/
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_description_3iCurculionidae(row['CharSp']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'description_name', language_id: lngsp) if !row['CharSp'].nil? && row['CharSp'] =~ /[\{\[]/
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_description_3iCurculionidae(row['CharZh']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'description_name', language_id: lngzh) if !row['CharZh'].nil? && row['CharSp'] =~ /[\{\[]/
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_key_3iCurculionidae(row['CharRu']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'key_name', language_id: lngru) if !row['CharRu'].nil? && row['CharRu'] =~ /[\{\[]/
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_key_3iCurculionidae(row['CharSp']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'key_name', language_id: lngsp) if !row['CharSp'].nil? && row['CharSp'] =~ /[\{\[]/
          a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_key_3iCurculionidae(row['CharZh']).capitalize, alternate_value_object: descriptor, alternate_value_object_attribute: 'key_name', language_id: lngzh) if !row['CharZh'].nil? && row['CharZh'] =~ /[\{\[]/
          ...
          unless row['KeyN'].blank?
            row['KeyN'].gsub(' ', '').split(',').each do |kn|
              #ObservationMatrixColumnItem::TaggedDescriptor.find_or_create_by!(observation_matrix_id: @data.keyn[kn], controlled_vocabulary_term_id: @data.morph[row['Morph']], project_id: Current.project_id) unless row['Morph'].blank?
              #ObservationMatrixColumnItem::TaggedDescriptor.find_or_create_by!(observation_matrix_id: @data.keyn[kn], controlled_vocabulary_term_id: @data.morph['n'], project_id: Current.project_id) if row['Type'].include?('n')
              #ObservationMatrixColumnItem::TaggedDescriptor.find_or_create_by!(observation_matrix_id: @data.keyn[kn], controlled_vocabulary_term_id: @data.morph['m'], project_id: Current.project_id) if row['Type'].include?('m')
              #ObservationMatrixColumnItem::TaggedDescriptor.find_or_create_by!(observation_matrix_id: @data.keyn[kn], controlled_vocabulary_term_id: @data.morph['f'], project_id: Current.project_id) if row['Type'].include?('f')
      ...
      def char_name_3iCurculionidae(string)
        string = string.to_s.gsub('{[+]}', '').gsub('{[!]}', '').gsub('{[-]}', '').gsub('[', '').gsub(']', '').gsub('{', '').gsub('}', '').gsub('&#176;', '°')
        string
      end

      def char_for_description_3iCurculionidae(string)
      ...
      def char_for_key_3iCurculionidae(string)
        string = string.to_s.gsub('{[+]}', '').gsub('{[!]}', '').gsub('{[-]}', '')
        4.times do
          matchdata = string.match(/^.*(\[.*\]).*$/)
          string = string.gsub(matchdata[1], '') unless matchdata.nil?
      ...
      def handle_morph_3iCurculionidae
        # Morph
        # MorphEn
        # MorphRu
        # MorphDt
        ...
        file = CSV.foreach(path, col_sep: "\t", headers: true)
        file.each_with_index do |row, i|
          kw = Keyword.find_or_create_by(name: row['MorphEn'], definition: 'Morphological group of descriptors: ' + row['MorphEn'].to_s + ' (' + row['Morph'].to_s + ')', project_id: Current.project_id)
          @data.morph[row['Morph'].to_s] = kw.id
        end
        kw1 = Keyword.find_or_create_by(name: 'Nymphs', definition: 'Morphological group of descriptors: Nymphs', project_id: Current.project_id)
        @data.morph['n'] = kw1.id unless kw1.blank?
        kw2 = Keyword.find_or_create_by(name: 'Males', definition: 'Morphological group of descriptors: Males', project_id: Current.project_id)
        @data.morph['m'] = kw2.id unless kw2.blank?
        kw3 = Keyword.find_or_create_by(name: 'Females', definition: 'Morphological group of descriptors: Females', project_id: Current.project_id)
        @data.morph['f'] = kw3.id unless kw3.blank?
      end

      def handle_state_3iCurculionidae
        # Key2
        # State
        # Key1
        # StateRu
        ...
        file.each do |row|
          ...
          if @data.chars[row['Key1'].to_s][1] == '1'
            ...
            cs = CharacterState.create(name: char_name_3iCurculionidae(row['StateEn']), description_name: char_for_description_3iCurculionidae(row['StateEn']), key_name: char_for_key_3iCurculionidae(row['StateEn']), label: row['State'], descriptor_id: @data.chars[row['Key1']][0], position: row['State'].to_i + 1)
            ...
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_name_3iCurculionidae(row['StateRu']), alternate_value_object: cs, alternate_value_object_attribute: 'name', language_id: lngru) unless row['StateRu'].nil?
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_name_3iCurculionidae(row['StateSp']), alternate_value_object: cs, alternate_value_object_attribute: 'name', language_id: lngsp) unless row['StateSp'].nil?
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_name_3iCurculionidae(row['StateZh']), alternate_value_object: cs, alternate_value_object_attribute: 'name', language_id: lngzh) unless row['StateZh'].nil?
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_description_3iCurculionidae(row['StateRu']), alternate_value_object: cs, alternate_value_object_attribute: 'description_name', language_id: lngru) if !row['StateRu'].nil? && row['StateRu'] =~ /[\{\[]/
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_description_3iCurculionidae(row['StateSp']), alternate_value_object: cs, alternate_value_object_attribute: 'description_name', language_id: lngsp) if !row['StateSp'].nil? && row['StateSp'] =~ /[\{\[]/
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_description_3iCurculionidae(row['StateZh']), alternate_value_object: cs, alternate_value_object_attribute: 'description_name', language_id: lngzh) if !row['StateZh'].nil? && row['StateZh'] =~ /[\{\[]/
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_key_3iCurculionidae(row['StateRu']), alternate_value_object: cs, alternate_value_object_attribute: 'key_name', language_id: lngru) if !row['StateRu'].nil? && row['StateRu'] =~ /[\{\[]/
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_key_3iCurculionidae(row['StateSp']), alternate_value_object: cs, alternate_value_object_attribute: 'key_name', language_id: lngsp) if !row['StateSp'].nil? && row['StateSp'] =~ /[\{\[]/
            a = AlternateValue.create(type: 'AlternateValue::Translation', value: char_for_key_3iCurculionidae(row['StateZh']), alternate_value_object: cs, alternate_value_object_attribute: 'key_name', language_id: lngzh) if !row['StateZh'].nil? && row['StateZh'] =~ /[\{\[]/
      ...
      def handle_chartable_3iCurculionidae
        # Key
        # Key2
        # NumericFrom
        # NumericTo
        ...
        file.each do |row|
          ...
          if @data.states[row['Key2'].to_s][2].nil? # Qualitative
            o = Observation::Qualitative.create(descriptor_id: @data.states[row['Key2'].to_s][1], otu_id: find_otu_3iCurculionidae(row['Key']).id, character_state_id: @data.states[row['Key2'].to_s][0] )
            byebug if o.id.nil?
          else # Sample
          o = Observation::Sample.create(descriptor_id: @data.states[row['Key2'].to_s][1], otu_id: find_otu_3iCurculionidae(row['Key']).id, sample_min: row['NumericFrom'].to_f, sample_max: row['NumericTo'].to_f, sample_units: @data.states[row['Key2'].to_s][2])
          byebug if o.id.nil?
          end
        end
      end

      def handle_content_types_3iCurculionidae
        # id
        # sti_type
        # name
        # can_markup
        ...
        file.each do |row|
          ...
          print "\r#{i}"
          @data.contents.merge!(
              row['id'].to_s => Topic.find_or_create_by!(name: row['name'].to_s, definition: 'OUT topic ' + row['name'].to_s + ' from mx Membracidae', project_id: Current.project_id).id )
        end
      end

      def handle_contents_3iCurculionidae
        # Key
        # id
        # otu_id
        # content_type_id
        ...
        file.each do |row|
          ...
          if row['pub_content_id'].blank? && !row['Key'].blank? && !find_otu_3iCurculionidae(row['Key']).blank? && !@data.contents[row['content_type_id']].blank? && !row['text'].blank?
            c = Content.find_or_create_by(text: row['text'],
                                          otu_id: find_otu_3iCurculionidae(row['Key']).id,
      ...
      def soft_validations_3iCurculionidae
        @data = nil
        GC.start
        fixed = 0
      ...
      def handle_trivellone_unique_species_3iCurculionidae
        # species
        # Key
      ...
      def handle_trivellone_phytoplasma_group_3iCurculionidae
        # PK_Phy
        # FK_Taxo_phy
        # 16Sr_group
        # 16Sr_subgroup
      ...
      def handle_trivellone_phytoplasma_taxonomy_3iCurculionidae
        # PK_Taxo_phy
        # FK_reference
        # FK_Phy
        # domain
        ...
        bacteria = Protonym.find_or_create_by!(name: 'Bacteria', rank_class: Ranks.lookup(:icnp, 'kingdom'), parent: @root, project_id: Current.project_id)
        tenericutes = Protonym.find_or_create_by!(name: 'Tenericutes', rank_class: Ranks.lookup(:icnp, 'phylum'), parent: bacteria, project_id: Current.project_id)
        mollicutes = Protonym.find_or_create_by!(name: 'Mollicutes', rank_class: Ranks.lookup(:icnp, 'class'), parent: tenericutes, project_id: Current.project_id)
        acholeplasmatales = Protonym.find_or_create_by!(name: 'Acholeplasmatales', rank_class: Ranks.lookup(:icnp, 'order'), parent: mollicutes, project_id: Current.project_id)
        acholeplasmataceae = Protonym.find_or_create_by!(name: 'Acholeplasmataceae', rank_class: Ranks.lookup(:icnp, 'family'), parent: acholeplasmatales, project_id: Current.project_id)
        @phytoplasma = Protonym.find_or_create_by!(name: 'Phytoplasma', rank_class: Ranks.lookup(:icnp, 'genus'), parent: acholeplasmataceae, project_id: Current.project_id)
        ...
        file.each do |row|
          ...
          if row['species'].blank?
            ...
            taxon = Protonym.find_or_create_by!(name: row['species'], rank_class: Ranks.lookup(:icnp, 'species'), parent: @phytoplasma, project_id: Current.project_id)
          ...
          otu = Otu.find_or_create_by!(name: name, taxon_name: taxon, project_id: Current.project_id)
          source_id = find_t_publication_id_3iCurculionidae(row['FK_reference'])
          otu.citations.find_or_create_by!(source_id: source_id, project_id: Current.project_id)
      ...
      def handle_trivellone_phytoplasma_otu_3iCurculionidae
        # PK_Phy
        # 16Sr_group
        # 16Sr_subgroup
        path = @args[:data_directory] + 'trivellone_phytoplasma_group.txt'
        ...
        file.each do |row|
          ...
          if name == 'neg'
            ...
            otu = Otu.find_or_create_by!(name: name, project_id: Current.project_id)
      ...
      def find_t_otu_id_3iCurculionidae(pk_phy)
        #r = @data.t_phyto_taxonomy(pk_phy)
        #return r unless r.nil?
        r = Identifier.find_by(cached: 'PK_Phy ' + pk_phy.to_s, project_id: Current.project_id)
        return nil if r.nil?
      ...
      def handle_trivellone_insect_phytoplasma_3iCurculionidae
        ...
        confidence = {
            'negative' => ConfidenceLevel.find_or_create_by(name: 'Negative', definition: 'Vector status is negative', project_id: Current.project_id).id,
            'suspected' => ConfidenceLevel.find_or_create_by(name: 'Suspected', definition: 'Vector status is suspected', project_id: Current.project_id).id,
            'potential' => ConfidenceLevel.find_or_create_by(name: 'Potential', definition: 'Vector status is potential', project_id: Current.project_id).id,
            'competent' => ConfidenceLevel.find_or_create_by(name: 'Competent', definition: 'Vector status is competent', project_id: Current.project_id).id}.freeze
        ...
        file.each do |row|
          ...
          s = find_t_publication_id_3iCurculionidae(row['FK_reference'])
          phy = nil
          phy = find_t_otu_id_3iCurculionidae(row['FK_Phy']) unless @phytoplasma_neg == row['FK_Phy']
          insect = find_otu_3iCurculionidae(@data.t_unique[row['species']])
          ...
          if insect
            ...
            c = Citation.find_or_create_by!(citation_object: ba, source_id: s, project_id: Current.project_id) unless s.blank?

            #            c.notes.create(text: row['note']) unless row['note'].blank?
            d = nil
            d = ba.notes.find_or_create_by!(text: row['note']) unless row['note'].blank?
            Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?


            c.citation_topics.find_or_create_by(topic: @data.topics['Infection_status_negative'], project_id: Current.project_id) if @phytoplasma_neg == row['FK_Phy']
            da.each do |ia|
              #              c.data_attributes.create(type: 'InternalAttribute', predicate: @data.keywords[ia], value: row[ia]) unless row[ia].blank?
              d = nil
              d = ba.data_attributes.find_or_create_by!(type: 'InternalAttribute', controlled_vocabulary_term_id: @data.keywords[ia].id, value: row[ia], project_id: Current.project_id) unless row[ia].blank?
              Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?
            end
            ...
            d = nil
            d = ba.tags.find_or_create_by!(keyword_id: @data.keywords['test_infection_positive'].id, project_id: Current.project_id) if row['test_infection'] == 'positive'
            d = ba.tags.find_or_create_by!(keyword_id: @data.keywords['test_infection_negative'].id, project_id: Current.project_id) if row['test_infection'] == 'negative'
            d = ba.tags.find_or_create_by!(keyword_id: @data.keywords['test_infection_suspected'].id, project_id: Current.project_id) if row['test_infection'] == 'suspected'
            Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?
            ...
            d = ba.tags.find_or_create_by!(keyword_id: @data.keywords['inoculation_trial_positive'].id, project_id: Current.project_id) if row['inoculation_trial'] == 'positive'
            d = ba.tags.find_or_create_by!(keyword_id: @data.keywords['inoculation_trial_negative'].id, project_id: Current.project_id) if row['inoculation_trial'] == 'negative'
            Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?

            habitat = [row['habitat 1'], row['habitat 2']].compact.join(': ')
            d = ba.data_attributes.find_or_create_by!(type: 'InternalAttribute', controlled_vocabulary_term_id: @data.keywords['hab'].id, value: habitat, project_id: Current.project_id) unless habitat.blank?
            Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?

            test_infection = row['test_infection'].to_s + ' (' + row['positive'].to_s + '/' + row['tested'].to_s + ')'
            d = ba.data_attributes.find_or_create_by!(type: 'InternalAttribute', controlled_vocabulary_term_id: @data.keywords['test_inf'].id, value: test_infection, project_id: Current.project_id) unless test_infection.blank?
            Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?
            ...
            d = ba.data_attributes.find_or_create_by!(type: 'InternalAttribute', controlled_vocabulary_term_id: @data.keywords['test_ino'].id, value: test_inoculation, project_id: Current.project_id) unless test_inoculation.blank?
            Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?
            ...
            d = nil
            d = ba.data_attributes.find_or_create_by!(type: 'InternalAttribute', controlled_vocabulary_term_id: @data.keywords['FK_idTW_g'].id, value: g.name, project_id: Current.project_id) unless g.nil?
            Citation.find_or_create_by!(citation_object: d, source_id: s, project_id: Current.project_id) if !s.blank? && !d.nil?
            ...
            if !insect.nil? && !s.nil? && !g.nil?
              ad = AssertedDistribution.find_or_create_by(
                  ...
                  geographic_area: g,
                  project_id: Current.project_id )
              c1 = ad.citations.new(source_id: s, project_id: Current.project_id)
              ad.save
            end
      ...
      def handle_trivellone_plant_phytoplasma_3iCurculionidae
        # PK_PlantPhyt
        # FK_Phy
        # FK_idTW
        # FK_reference
        ...
        file.each do |row|
          ...
          s = find_t_publication_id_3iCurculionidae(row['FK_reference'])
          phy = find_t_otu_id_3iCurculionidae(row['FK_Phy'])
