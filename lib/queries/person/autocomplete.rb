        ...
        queries.each_with_index do |q ,i|
          if roles_assigned?
            ...
            a = q.left_outer_joins(:roles)
                  ...
                  .select('people.*, COUNT(roles.id) AS role_count, CASE WHEN MAX(roles.project_id) > 0 THEN MAX(roles.project_id) ELSE MAX(project_sources.project_id) END AS in_project_id')
                  ...
                  .group('people.id, roles.project_id')
                  .order('in_project_id, role_count DESC')
          end
          a ||= q
          updated_queries[i] = a
        end
