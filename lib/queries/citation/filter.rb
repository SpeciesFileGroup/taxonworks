  ...
  module Citation
    ...
    class Filter < Query::Filter
      ...
      def anatomical_part_query_facet
        polymorphic_annotation_object_query_facet(anatomical_part_query, 'AnatomicalPart', 'query_ap_c')
      end

      def asserted_distribution_query_facet
        polymorphic_annotation_object_query_facet(asserted_distribution_query, 'AssertedDistribution', 'query_ad_c')
      end
      ...
      def collecting_event_query_facet
        polymorphic_annotation_object_query_facet(collecting_event_query, 'CollectingEvent', 'query_ce_c')
      end

      def collection_object_query_facet
        polymorphic_annotation_object_query_facet(collection_object_query, 'CollectionObject', 'query_co_c')
      end

      def content_query_facet
        polymorphic_annotation_object_query_facet(content_query, 'Content', 'query_cnt_c')
      end

      def descriptor_query_facet
        polymorphic_annotation_object_query_facet(descriptor_query, 'Descriptor', 'query_d_c')
      end

      def extract_query_facet
        polymorphic_annotation_object_query_facet(extract_query, 'Extract', 'query_ex_c')
      end

      def field_occurrence_query_facet
        polymorphic_annotation_object_query_facet(field_occurrence_query, 'FieldOccurrence', 'query_fo_c')
      end

      def image_query_facet
        polymorphic_annotation_object_query_facet(image_query, 'Image', 'query_im_c')
      end

      def observation_query_facet
        polymorphic_annotation_object_query_facet(observation_query, 'Observation', 'query_obs_c')
      end

      def otu_query_facet
        polymorphic_annotation_object_query_facet(otu_query, 'Otu', 'query_otu_c')
      end

      def sound_query_facet
        polymorphic_annotation_object_query_facet(sound_query, 'Sound', 'query_snd_c')
      end

      def taxon_name_query_facet
        polymorphic_annotation_object_query_facet(taxon_name_query, 'TaxonName', 'query_tn_c')
      end

      def taxon_name_relationship_query_facet
        polymorphic_annotation_object_query_facet(taxon_name_relationship_query, 'TaxonNameRelationship', 'query_tnr_c')
      end
