...
module Housekeeping::Timestamps
  ...
  included do
    ...
    scope :created_in_range, ->(start, stop) { where('created_at > ? and created_at < ?', start, stop) }
    scope :updated_in_range, ->(start, stop) { where('updated_at > ? and updated_at < ?', start, stop) }
  end
